...
let mysqlLoadPromise: Promise<void> | null = null;
...
export function initMysqlRunner(): Promise<void> {
  if (mysqlReady) return Promise.resolve();
  if (mysqlLoadPromise) return mysqlLoadPromise;

  mysqlLoadPromise = (async () => {
    try {
...
export async function runMysql(code: string): Promise<RunResult> {
  if (!mysqlReady || !SQL) {
    return {
      stdout: "",
      stderr: "",
      error: "MySQL runtime is not loaded.",
    };
  }
