...
export const completeMiniLanguage: Lesson = {
	id: "complete-mini-language",
	title: "Complete Mini-Language",
	chapterId: "advanced",
...
\t\t}
\t\t// TODO: handle "while condition do body;"
...
interpret('let i = 1; while i < 4 do let i = i + 1; print i;');
interpret('fn factorial(n) = if n < 2 then 1 else n * factorial(n - 1); print factorial(5);');
interpret('let msg = "hello" + " " + "world"; print msg;');
`,
...
\t\t\treturn { type: "FnStatement", name, params, body };
\t\t}
\t\tif (this.peek().type === "IDENT" && this.peek().value === "while") {
\t\t\tthis.advance();
\t\t\tconst condition = this.parseExpr();
\t\t\tthis.advance(); // consume 'do'
\t\t\tconst body = this.parseStatement();
...
`,
	...
	tests: [
		{
			name: "while loop, recursion, and strings",
			expected: "4\n120\nhello world\n",
		},
		{
			name: "while loop counting",
			code: `{{FUNC}}
interpret('let x = 0; while x < 10 do let x = x + 2; print x;');`,
			expected: "10\n",
		},
		{
			name: "fibonacci with recursion",
			code: `{{FUNC}}
interpret('fn fib(n) = if n < 2 then n else fib(n - 1) + fib(n - 2); print fib(10);');`,
			expected: "55\n",
		},
	],
};
