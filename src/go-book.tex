...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Functions} --- Declaring functions, multiple return values, named returns, and \code{defer}.
  \item \textbf{Data Structures} --- Slices (dynamic arrays), maps (hash tables), and strings in depth.
  \item \textbf{Custom Types} --- Pointers, structs with methods, interfaces, type assertions, and structural typing.
  ...
  \item \textbf{Concurrency} --- Goroutines, channels, and the \code{select} statement.
\end{itemize}
...
\begin{taskbox}
Write a function \code{fizzBuzz} that takes an integer \code{n} and prints the numbers from 1 to \code{n} (inclusive), one per line, with these substitutions:
\begin{itemize}[leftmargin=1.5em]
...
\subsection*{Cleaning Up After Yourself}

The \code{defer} keyword schedules a function call to run when the surrounding function returns. It is Go's way of ensuring cleanup happens no matter how the function exits.
...
The deferred call runs \emph{after} the function body completes but \emph{before} it returns to the caller.

\subsubsection*{LIFO Order}

When you defer multiple calls, they execute in \textbf{last-in, first-out} order --- like a stack:
...
\subsubsection*{Common Pattern: Open Then Defer Close}

The most common use of \code{defer} is pairing resource acquisition with cleanup on the very next line:
...
\subsubsection*{Arguments Are Evaluated Immediately}

The arguments to a deferred call are evaluated when the \code{defer} statement executes, not when the deferred function runs:
...
\subsubsection*{Defer in Loops}

Be careful with \code{defer} inside loops. Deferred calls accumulate and only run when the function returns, not when the loop iteration ends:
...
\begin{taskbox}
Write a function \code{countdown(n int)} that uses \code{defer} inside a loop to print numbers in reverse order, followed by \code{"Go!"}.

For example, \code{countdown(3)} should print \code{3}, \code{2}, \code{1}, \code{Go!} (each on its own line).

Hint: defer each number inside the loop. Since deferred calls execute in LIFO order, deferring 1, 2, 3 will print 3, 2, 1. Print \code{"Go!"} using a defer \emph{before} the loop.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \code{filterEvens} --- takes a \code{[]int} and returns a new \code{[]int} containing only the even numbers, in order.
\end{itemize}
...
\subsubsection*{Bytes vs Runes}

A \code{string} in Go is a read-only slice of bytes. The \code{len} function returns the number of \textbf{bytes}, not the number of characters:
...
Go uses the term \textbf{rune} for what most languages call a character. A rune is an alias for \code{int32} and represents a single Unicode code point.

To get the actual character count, use \code{utf8.RuneCountInString}:
...
\subsubsection*{Iterating Over Strings}

A \code{for range} loop over a string yields runes, not bytes:
...
The index is the byte position, but \code{r} is a full rune. This is the safe way to process strings character by character.

\subsubsection*{The strings Package}

The \code{strings} package provides essential string operations:
...
\code{strings.Fields} splits on any whitespace and ignores leading/trailing spaces. It is often more useful than \code{strings.Split}.
...
For number conversions, use the \code{strconv} package:
...
\begin{taskbox}
Write a function \code{acronym(s string) string} that takes a phrase and returns its acronym. Take the first letter of each word and uppercase it.

For example, \code{"Portable Network Graphics"} becomes \code{"PNG"}.

Use \code{strings.Fields} to split into words, and \code{strings.ToUpper} to uppercase.
...
When you have a value of type \code{any} (or any interface), you can extract the underlying concrete value using a \textbf{type assertion}:
...
\subsubsection*{Type Switches}

A \textbf{type switch} lets you branch based on the concrete type of an interface value:
...
Inside each case, \code{v} is already the correct type --- no further assertion needed. Type switches are cleaner than chains of type assertions when you need to handle multiple types.
...
Write a function \code{describeShape} that takes a \code{Shape} and returns a string using a type switch:
\begin{itemize}[leftmargin=1.5em]
  \item For a \code{Circle}, return \code{"circle with radius X.XX"}
  \item For a \code{Square}, return \code{"square with side X.XX"}
  \item For anything else, return \code{"unknown shape"}
\end{itemize}
...
% ─── Lesson 16: Goroutines and Channels ──────────────────────────
...
By default, channels are \textit{unbuffered}: a send blocks until another goroutine receives. A \textbf{buffered channel} has a capacity, allowing sends to proceed without a receiver until the buffer is full:
...
\subsubsection*{Synchronization with Channels}
...
For waiting on multiple goroutines, use \code{sync.WaitGroup}. Call \code{Add} before launching, \code{Done} when each goroutine finishes, and \code{Wait} to block until all are done:
...
\subsection*{Multiplexing Channels}

The \code{select} statement lets a goroutine wait on multiple channel operations simultaneously. It is like a \code{switch} statement, but each case is a channel send or receive.
...
If multiple channels are ready, \code{select} picks one at random. If none are ready, it blocks until one becomes ready.

\subsubsection*{Non-Blocking Operations}

Add a \code{default} case to make channel operations non-blocking:
...
Without \code{default}, the \code{select} blocks. With \code{default}, it executes the default case immediately if no channel is ready.

\subsubsection*{Pattern: Merging Channels}

A common use of \code{select} is merging multiple channels into one. This lets a consumer read from a single channel regardless of how many producers exist:
...
Setting a channel to \code{nil} after it closes prevents \code{select} from receiving zero values from it, because receives on a nil channel block forever.

\begin{taskbox}
Write a function \code{merge(ch1, ch2 <-chan int) <-chan int} that merges two integer channels into one. The output channel should receive all values from both input channels and close when both inputs are exhausted.

Key steps:
\begin{enumerate}[leftmargin=1.5em]
  \item Create an output channel and launch a goroutine
  \item Loop while at least one input channel is still open
  \item Use \code{select} to receive from whichever channel is ready
  \item When a channel is closed (\code{ok} is false), set it to \code{nil} so \code{select} ignores it
  \item Close the output channel when both inputs are exhausted
\end{enumerate}
