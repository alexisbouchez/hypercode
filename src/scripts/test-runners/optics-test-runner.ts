...
import { opticsLessons } from "@/lib/lessons/optics/index";
import type { LessonTestResult } from "./types";

export function runOpticsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of opticsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_optics_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
