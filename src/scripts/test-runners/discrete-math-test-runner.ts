...
import { discreteMathLessons } from "@/lib/lessons/discrete-math/index";
import type { LessonTestResult } from "./types";

export function runDiscreteMathTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of discreteMathLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_dm_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
