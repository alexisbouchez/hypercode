...
import { distributedSystemsLessons } from "@/lib/lessons/distributed-systems/index";
import type { LessonTestResult } from "./types";

export function runDistributedSystemsTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  const tmp = join(tmpdir(), `hypercode-distributed-systems-${Date.now()}`);
  mkdirSync(tmp, { recursive: true });

  try {
    for (const lesson of distributedSystemsLessons) {
