import { calculus3Lessons } from "@/lib/lessons/calculus3/index";
...
function compileCToElf(source: string): Uint8Array {
	...
	if (exitCode !== 0) throw new Error("Compilation failed");
...
function elfToAssembly(elfBytes: Uint8Array): string {
	...
	if (!elf.textSection) throw new Error("No .text section in compiled output");
...
export async function runCalculus3Tests(): Promise<LessonTestResult[]> {
	await initTCC();

	const results: LessonTestResult[] = [];

	for (const lesson of calculus3Lessons) {
