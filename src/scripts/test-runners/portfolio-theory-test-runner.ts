...
import { portfolioTheoryLessons } from "@/lib/lessons/portfolio-theory/index";
import type { LessonTestResult } from "./types";

export function runPortfolioTheoryTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  for (const lesson of portfolioTheoryLessons) {
    for (const test of lesson.tests) {
      ...
      const tmpFile = join(
        tmpdir(),
        `hypercode_portfolio_theory_${Date.now()}_${Math.random().toString(36).slice(2)}.py`
      );
      try {
