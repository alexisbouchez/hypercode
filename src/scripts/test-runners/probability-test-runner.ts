...
import { probabilityLessons } from "@/lib/lessons/probability/index";
import type { LessonTestResult } from "./types";

export function runProbabilityTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of probabilityLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_prob_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
