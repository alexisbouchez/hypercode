...
import { electromagnetismLessons } from "@/lib/lessons/electromagnetism/index";
import type { LessonTestResult } from "./types";

export function runElectromagnetismTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of electromagnetismLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_em_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
