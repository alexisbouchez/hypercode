...
import { generalRelativityLessons } from "@/lib/lessons/general-relativity/index";
import type { LessonTestResult } from "./types";

export function runGeneralRelativityTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of generalRelativityLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_gr_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
