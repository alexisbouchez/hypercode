...
import { financialMathLessons } from "@/lib/lessons/financial-math/index";
import type { LessonTestResult } from "./types";

export function runFinancialMathTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];

  for (const lesson of financialMathLessons) {
    for (const test of lesson.tests) {
      ...
      const tmpFile = join(
        tmpdir(),
        `hypercode_financial_math_${Date.now()}_${Math.random().toString(36).slice(2)}.py`
      );
