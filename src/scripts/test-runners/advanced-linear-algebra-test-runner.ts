...
import { advancedLinearAlgebraLessons } from "@/lib/lessons/advanced-linear-algebra/index";
import type { LessonTestResult } from "./types";

export function runAdvancedLinearAlgebraTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of advancedLinearAlgebraLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_adv_la_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
