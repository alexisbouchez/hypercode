...
import { realAnalysisLessons } from "@/lib/lessons/real-analysis/index";
import type { LessonTestResult } from "./types";

export function runRealAnalysisTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of realAnalysisLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_realanalysis_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
