...
import { quantStatsLessons } from "@/lib/lessons/quant-stats/index";
import type { LessonTestResult } from "./types";

export function runQuantStatsTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  for (const lesson of quantStatsLessons) {
    for (const test of lesson.tests) {
      ...
      const tmpFile = join(
        tmpdir(),
        `hypercode_quant_stats_${Date.now()}_${Math.random().toString(36).slice(2)}.py`
      );
      try {
