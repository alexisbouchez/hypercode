...
import { numberTheoryLessons } from "@/lib/lessons/number-theory/index";
import type { LessonTestResult } from "./types";

export function runNumberTheoryTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of numberTheoryLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_numtheory_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
