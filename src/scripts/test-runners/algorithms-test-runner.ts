...
import { algorithmsLessons } from "@/lib/lessons/algorithms/index";
import type { LessonTestResult } from "./types";

export function runAlgorithmsTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  const tmp = join(tmpdir(), `hypercode-algorithms-${Date.now()}`);
  mkdirSync(tmp, { recursive: true });

  try {
    for (const lesson of algorithmsLessons) {
