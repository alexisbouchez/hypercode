...
import { digitalLogicLessons } from "@/lib/lessons/digital-logic";
import type { LessonTestResult } from "./types";

export function runDigitalLogicTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  const tmp = join(tmpdir(), `hypercode-digital-logic-${Date.now()}`);
  mkdirSync(tmp, { recursive: true });

  try {
    for (const lesson of digitalLogicLessons) {
