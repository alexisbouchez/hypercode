...
import { informationTheoryLessons } from "@/lib/lessons/information-theory/index";
import type { LessonTestResult } from "./types";

export function runInformationTheoryTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of informationTheoryLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_infotheory_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
