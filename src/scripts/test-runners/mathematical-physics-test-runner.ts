...
import { mathematicalPhysicsLessons } from "@/lib/lessons/mathematical-physics/index";
import type { LessonTestResult } from "./types";

export function runMathematicalPhysicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of mathematicalPhysicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_mathphys_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
