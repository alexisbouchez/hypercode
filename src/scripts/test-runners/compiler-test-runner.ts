...
import { compilerLessons } from "@/lib/lessons/compiler/index";
import type { LessonTestResult } from "./types";

export function runCompilerTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  const tmp = join(tmpdir(), `hypercode-compiler-${Date.now()}`);
  mkdirSync(tmp, { recursive: true });

  try {
    for (const lesson of compilerLessons) {
