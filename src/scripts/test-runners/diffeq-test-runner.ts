...
import { diffeqLessons } from "@/lib/lessons/diffeq/index";
import type { LessonTestResult } from "./types";

export function runDiffeqTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of diffeqLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_diffeq_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
