...
import { abstractAlgebraLessons } from "@/lib/lessons/abstract-algebra/index";
import type { LessonTestResult } from "./types";

export function runAbstractAlgebraTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of abstractAlgebraLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_absalg_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
