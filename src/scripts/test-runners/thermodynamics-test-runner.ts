...
import { thermodynamicsLessons } from "@/lib/lessons/thermodynamics/index";
import type { LessonTestResult } from "./types";

export function runThermodynamicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of thermodynamicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_thermo_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
