...
import { nuclearPhysicsLessons } from "@/lib/lessons/nuclear-physics/index";
import type { LessonTestResult } from "./types";

export function runNuclearPhysicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of nuclearPhysicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_nuclear_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
