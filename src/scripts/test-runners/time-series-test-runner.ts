...
import { timeSeriesLessons } from "@/lib/lessons/time-series/index";
import type { LessonTestResult } from "./types";

export function runTimeSeriesTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  for (const lesson of timeSeriesLessons) {
    for (const test of lesson.tests) {
      ...
      const tmpFile = join(
        tmpdir(),
        `hypercode_time_series_${Date.now()}_${Math.random().toString(36).slice(2)}.py`
      );
      try {
