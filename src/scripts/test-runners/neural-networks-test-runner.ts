...
import { neuralNetworksLessons } from "@/lib/lessons/neural-networks/index";
import type { LessonTestResult } from "./types";

export function runNeuralNetworksTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of neuralNetworksLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_nn_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
