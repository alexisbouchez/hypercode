import { coreutilsLessons } from "@/lib/lessons/coreutils/index";
...
export async function runCoreutilsTests(): Promise<LessonTestResult[]> {
	await initTCC();

	const results: LessonTestResult[] = [];

	for (const lesson of coreutilsLessons) {
