...
import { cryptographyLessons } from "@/lib/lessons/cryptography/index";
import type { LessonTestResult } from "./types";

export function runCryptographyTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of cryptographyLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_crypto_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
