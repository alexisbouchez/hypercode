...
import { genomicsLessons } from "@/lib/lessons/genomics/index";
import type { LessonTestResult } from "./types";

export function runGenomicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of genomicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_genomics_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
