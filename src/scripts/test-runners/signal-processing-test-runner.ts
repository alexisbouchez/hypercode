...
import { signalProcessingLessons } from "@/lib/lessons/signal-processing/index";
import type { LessonTestResult } from "./types";

export function runSignalProcessingTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of signalProcessingLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_sigproc_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
