...
import { particlePhysicsLessons } from "@/lib/lessons/particle-physics/index";
import type { LessonTestResult } from "./types";

export function runParticlePhysicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of particlePhysicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_particle_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
