...
import { algoTradingLessons } from "@/lib/lessons/algo-trading/index";
import type { LessonTestResult } from "./types";

export function runAlgoTradingTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];

  for (const lesson of algoTradingLessons) {
    for (const test of lesson.tests) {
      ...
      const tmpFile = join(
        tmpdir(),
        `hypercode_algo_trading_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2)}.py`
      );
