...
import { cosmologyLessons } from "@/lib/lessons/cosmology/index";
import type { LessonTestResult } from "./types";

export function runCosmologyTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of cosmologyLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_cosmology_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
