...
import { statisticsLessons } from "@/lib/lessons/statistics/index";
import type { LessonTestResult } from "./types";

export function runStatisticsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of statisticsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_stats_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
