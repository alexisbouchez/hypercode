...
import { chaosTheoryLessons } from "@/lib/lessons/chaos-theory/index";
import type { LessonTestResult } from "./types";

export function runChaosTheoryTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of chaosTheoryLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_chaos_theory_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
