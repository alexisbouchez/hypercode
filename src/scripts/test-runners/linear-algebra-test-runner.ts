...
import { linearAlgebraLessons } from "@/lib/lessons/linear-algebra/index";
import type { LessonTestResult } from "./types";

export function runLinearAlgebraTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of linearAlgebraLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_la_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
