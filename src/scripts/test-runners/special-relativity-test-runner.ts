...
import { specialRelativityLessons } from "@/lib/lessons/special-relativity/index";
import type { LessonTestResult } from "./types";

export function runSpecialRelativityTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of specialRelativityLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_sr_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
