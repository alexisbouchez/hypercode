...
import { networkingLessons } from "@/lib/lessons/networking/index";
import type { LessonTestResult } from "./types";

export function runNetworkingTests(): LessonTestResult[] {
  const results: LessonTestResult[] = [];
  const tmp = join(tmpdir(), `hypercode-networking-${Date.now()}`);
