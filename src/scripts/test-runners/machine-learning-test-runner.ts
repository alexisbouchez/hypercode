...
import { machineLearningLessons } from "@/lib/lessons/machine-learning/index";
import type { LessonTestResult } from "./types";

export function runMachineLearningTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of machineLearningLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_ml_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
