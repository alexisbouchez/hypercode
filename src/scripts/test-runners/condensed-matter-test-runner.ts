...
import { condensedMatterLessons } from "@/lib/lessons/condensed-matter/index";
import type { LessonTestResult } from "./types";

export function runCondensedMatterTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of condensedMatterLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_condensed_matter_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
