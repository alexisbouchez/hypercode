...
import { advancedQuantumLessons } from "@/lib/lessons/advanced-quantum/index";
import type { LessonTestResult } from "./types";

export function runAdvancedQuantumTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of advancedQuantumLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_aq_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
