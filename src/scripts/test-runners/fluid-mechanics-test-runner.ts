...
import { fluidMechanicsLessons } from "@/lib/lessons/fluid-mechanics/index";
import type { LessonTestResult } from "./types";

export function runFluidMechanicsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of fluidMechanicsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_fluid_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
