...
import { pythonLessons } from "@/lib/lessons/python/index";
import type { LessonTestResult } from "./types";

export function runPythonTests(): LessonTestResult[] {
	...
	for (const lesson of pythonLessons) {
		for (const test of lesson.tests) {
			...
			const tmpFile = join(tmpdir(), `hypercode_python_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
