...
import { graphsLessons } from "@/lib/lessons/graphs/index";
import type { LessonTestResult } from "./types";

export function runGraphsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of graphsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_graphs_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
