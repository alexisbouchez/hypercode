...
import { complexSystemsLessons } from "@/lib/lessons/complex-systems/index";
import type { LessonTestResult } from "./types";

export function runComplexSystemsTests(): LessonTestResult[] {
	const results: LessonTestResult[] = [];

	for (const lesson of complexSystemsLessons) {
		for (const test of lesson.tests) {
			const codeToRun = test.code
				? test.code.replace("{{FUNC}}", extractPythonFunctions(lesson.solution))
				: lesson.solution;

			const tmpFile = join(tmpdir(), `hypercode_complex_systems_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
			try {
