...
import { Hypercode } from "../src/codex";
...
describe("Hypercode", () => {
  it("returns thread events", async () => {
    ...
    try {
      const client = new Hypercode({ codexPathOverride: codexExecPath, baseUrl: url, apiKey: "test" });
  ...
  it("allows overriding the env passed to the Hypercode CLI", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
