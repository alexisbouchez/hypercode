...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the hypercode tool call.
    ...
    pub hypercode_command: Vec<String>,
    pub hypercode_cwd: PathBuf,
    pub hypercode_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    event_id: String,
    call_id: String,
    hypercode_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Hypercode to run `{escaped_command}` in `{cwd}`?",
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(event_id, on_response, hypercode).await;
        });
    }
}
