...
use crate::hypercode_tool_config::HypercodeToolCallParam;
use crate::hypercode_tool_config::HypercodeToolCallReplyParam;
use crate::hypercode_tool_config::create_tool_for_hypercode_tool_call_param;
use crate::hypercode_tool_config::create_tool_for_hypercode_tool_call_reply_param;
...
use hypercode_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_hypercode_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            thread_manager,
            running_requests_id_to_hypercode_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "hypercode-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("Hypercode".to_string()),
                user_agent: Some(get_hypercode_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_hypercode_tool_call_param(),
                create_tool_for_hypercode_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "hypercode" => self.handle_tool_call_hypercode(id, arguments).await,
            "hypercode-reply" => {
                self.handle_tool_call_hypercode_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_hypercode(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<HypercodeToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.hypercode_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load Hypercode configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Hypercode tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for hypercode tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        let running_requests_id_to_hypercode_uuid = self.running_requests_id_to_hypercode_uuid.clone();

        // Spawn an async task to handle the Hypercode session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Hypercode session and stream events back to the client.
            crate::hypercode_tool_runner::run_hypercode_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_hypercode_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_hypercode_session_reply(
        &self,
        ...
        // parse arguments
        let hypercode_tool_call_reply_param: HypercodeToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<HypercodeToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Hypercode tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Hypercode tool: {e}"),
            ...
            None => {
                tracing::error!(
                    "Missing arguments for hypercode-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for hypercode-reply tool-call; the `thread_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        let thread_id = match hypercode_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let hypercode = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
        ...
        let prompt = hypercode_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_hypercode_uuid = running_requests_id_to_hypercode_uuid.clone();

            async move {
                crate::hypercode_tool_runner::run_hypercode_tool_session_reply(
                    thread_id,
                    hypercode,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_hypercode_uuid.lock().await;
        ...
        // Obtain the Hypercode thread from the server.
        let hypercode_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Hypercode.
        let err = hypercode_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: hypercode_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to Hypercode: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_hypercode_uuid
            .lock()
            .await
            .remove(&request_id);
    }
