...
use hypercode_core::parse_command;
use hypercode_core::protocol::FileChange;
use hypercode_core::protocol::ReviewDecision;
use hypercode_core::spawn::HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use hypercode_mcp_server::HypercodeToolCallParam;
use hypercode_mcp_server::ExecApprovalElicitRequestParams;
use hypercode_mcp_server::ExecApprovalResponse;
use hypercode_mcp_server::PatchApprovalElicitRequestParams;
use hypercode_mcp_server::PatchApprovalResponse;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Hypercode sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "hypercode" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let hypercode_request_id = mcp_process
        .send_hypercode_tool_call(HypercodeToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        hypercode_request_id.to_string(),
        params.hypercode_event_id.clone(),
        params.thread_id,
    )?;
    ...
    // Verify the original `hypercode` tool call completes and that the file was created.
    let hypercode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(hypercode_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(hypercode_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    hypercode_mcp_tool_call_id: String,
    hypercode_event_id: String,
    thread_id: hypercode_protocol::ThreadId,
) -> anyhow::Result<JSONRPCRequest> {
    let expected_message = format!(
        "Allow Hypercode to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let hypercode_parsed_cmd = parse_command::parse_command(&command);
    Ok(JSONRPCRequest {
        jsonrpc: JSONRPC_VERSION.into(),
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            thread_id,
            hypercode_elicitation: "exec-approval".to_string(),
            hypercode_mcp_tool_call_id,
            hypercode_event_id,
            hypercode_command: command,
            hypercode_cwd: workdir.to_path_buf(),
            hypercode_call_id: "call1234".to_string(),
            hypercode_parsed_cmd,
        })?),
    })
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "hypercode" tool request that will trigger the apply_patch command
    let hypercode_request_id = mcp_process
        .send_hypercode_tool_call(HypercodeToolCallParam {
    ...
    // Verify the original `hypercode` tool call completes
    let hypercode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(hypercode_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_hypercode_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = hypercode_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn hypercode_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `hypercode mcp` with a specific config.toml.
    let hypercode_home = TempDir::new()?;
    create_config_toml(hypercode_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(hypercode_home.path()).await?;
    ...
    .await??;
    assert_eq!(hypercode_response.jsonrpc, JSONRPC_VERSION);
    assert_eq!(hypercode_response.id, RequestId::Integer(hypercode_request_id));
    assert_eq!(
        hypercode_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": hypercode_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("hypercode tool response should include structuredContent.threadId"),
            }
        })
    );
...
/// Create a Hypercode config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(hypercode_home: &Path, server_uri: &str) -> std::io::Result<()> {
