...
use hypercode_otel::metrics::Result;
...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "hypercode-cli"), ("env", "prod")])?;

    metrics.counter("hypercode.turns", 1, &[("model", "gpt-5.1"), ("env", "dev")])?;
    metrics.histogram("hypercode.tool_latency", 25, &[("tool", "shell")])?;
    metrics.shutdown()?;

    let resource_metrics = latest_metrics(&exporter);

    let counter = find_metric(&resource_metrics, "hypercode.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "hypercode-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "hypercode.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "hypercode.tool_latency").and_then(|metric| {
            match metric.data() {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[
        ("service", "hypercode-cli"),
        ("env", "prod"),
        ("region", "us"),
    ])?;

    metrics.counter("hypercode.alpha", 1, &[("env", "dev"), ("component", "alpha")])?;
    metrics.counter(
        "hypercode.beta",
        2,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "hypercode.alpha").expect("hypercode.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "hypercode.beta").expect("hypercode.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("hypercode.turns", 1, &[])?;
    metrics.shutdown()?;
