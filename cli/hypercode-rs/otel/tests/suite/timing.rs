...
fn record_duration_records_histogram() -> Result<()> {
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&latest_metrics(&exporter), "hypercode.request_latency");
...
fn timer_result_records_success() -> Result<()> {
    ...
    {
        let timer = metrics.start_timer("hypercode.request_latency", &[("route", "chat")]);
        assert!(timer.is_ok());
    }
    ...
    let (bounds, bucket_counts, _sum, count) =
        histogram_data(&resource_metrics, "hypercode.request_latency");
    assert!(!bounds.is_empty());
    assert_eq!(count, 1);
    assert_eq!(bucket_counts.iter().sum::<u64>(), 1);
    let attrs = attributes_to_map(
        match crate::harness::find_metric(&resource_metrics, "hypercode.request_latency").and_then(
