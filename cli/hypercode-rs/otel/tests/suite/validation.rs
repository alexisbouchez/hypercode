use hypercode_otel::metrics::MetricsClient;
use hypercode_otel::metrics::MetricsConfig;
use hypercode_otel::metrics::MetricsError;
use hypercode_otel::metrics::Result;
use opentelemetry_sdk::metrics::InMemoryMetricExporter;

fn build_in_memory_client() -> Result<MetricsClient> {
    let exporter = InMemoryMetricExporter::default();
    let config = MetricsConfig::in_memory("test", "hypercode-cli", env!("CARGO_PKG_VERSION"), exporter);
...
fn counter_rejects_invalid_tag_key() -> Result<()> {
    ...
    let err = metrics
        .counter("hypercode.turns", 1, &[("bad key", "value")])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::InvalidTagComponent { label, value }
...
fn histogram_rejects_invalid_tag_value() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics
        .histogram("hypercode.request_latency", 3, &[("route", "bad value")])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::InvalidTagComponent { label, value }
...
fn counter_rejects_negative_increment() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics.counter("hypercode.turns", -1, &[]).unwrap_err();
    assert!(matches!(
        err,
        MetricsError::NegativeCounterIncrement { name, inc } if name == "hypercode.turns" && inc == -1
    ));
    metrics.shutdown()?;
    Ok(())
}
