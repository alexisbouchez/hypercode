...
```shell
cargo run -p hypercode-execpolicy-legacy -- check ls -l foo | jq
```
...
Further, "safety" in this system is not a guarantee that the command will execute successfully. As an example, `cat /Users/mbolin/code/hypercode/README.md` may be considered "safe" if the system has decided the agent is allowed to read anything under `/Users/mbolin/code/hypercode`, but it will fail at runtime if `README.md` does not exist. (Though this is "safe" in that the agent did not read any files that it was not authorized to read.)
...
```shell
cargo run -p hypercode-execpolicy-legacy -- check cp src1 src2 dest | jq
```
...
```shell
cargo run -p hypercode-execpolicy-legacy -- check applied deploy | jq
```
