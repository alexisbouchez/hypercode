...
const RESOURCE_URI: &str = "memo://hypercode/example-note";

fn stdio_server_bin() -> Result<PathBuf, CargoBinError> {
    hypercode_utils_cargo_bin::cargo_bin("test_stdio_server")
}
...
fn init_params() -> InitializeRequestParams {
    InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "hypercode-test".into(),
            version: "0.0.0-test".into(),
            title: Some("Hypercode rmcp resource test".into()),
            user_agent: None,
        },
    ...
    assert_eq!(
        ...
        ListResourceTemplatesResult {
            ...
            resource_templates: vec![ResourceTemplate {
                annotations: None,
                description: Some(
                    "Template for memo://hypercode/{slug} resources used in tests.".to_string()
                ),
                mime_type: Some("text/plain".to_string()),
                name: "hypercode-memo".to_string(),
                title: Some("Hypercode Memo".to_string()),
                uri_template: "memo://hypercode/{slug}".to_string(),
            }],
        }
    );
