...
//!
//! If the keyring is not available or fails, we fall back to HYPERCODE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use crate::find_hypercode_home::find_hypercode_home;

const KEYRING_SERVICE: &str = "Hypercode MCP Credentials";
...
/// Determine where Hypercode should store and read MCP credentials.
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// HYPERCODE_HOME/.credentials.json
...
fn fallback_file_path() -> Result<PathBuf> {
    let mut path = find_hypercode_home()?;
...
mod tests {
    ...
    struct TempHypercodeHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempHypercodeHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create HYPERCODE_HOME temp dir");
            unsafe {
                std::env::set_var("HYPERCODE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempHypercodeHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("HYPERCODE_HOME");
            }
        }
    }
    ...
    fn load_oauth_tokens_reads_from_keyring_when_available() -> Result<()> {
        let _env = TempHypercodeHome::new();
