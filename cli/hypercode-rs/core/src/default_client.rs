...
pub use hypercode_client::HypercodeRequestBuilder;
...
/// See https://github.com/openai/hypercode/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Hypercode. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "hypercode_cli_rs";
...
pub fn get_hypercode_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Hypercode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Hypercode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Hypercode originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> HypercodeHttpClient {
    let inner = build_reqwest_client();
    HypercodeHttpClient::new(inner)
}
...
pub fn build_reqwest_client() -> reqwest::Client {
    ...
    let ua = get_hypercode_user_agent();
...
fn is_sandboxed() -> bool {
    std::env::var(HYPERCODE_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    #[test]
    fn test_get_hypercode_user_agent() {
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        // User-Agent matches the computed Hypercode UA for that originator
        let expected_ua = get_hypercode_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "hypercode_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "hypercode_cli_rs/0.0.0 (bad_suffix)"
        );
    }
