...
use hypercode_execpolicy::AmendError;
use hypercode_execpolicy::Decision;
use hypercode_execpolicy::Error as ExecPolicyRuleError;
use hypercode_execpolicy::Evaluation;
use hypercode_execpolicy::Policy;
use hypercode_execpolicy::PolicyParser;
use hypercode_execpolicy::RuleMatch;
use hypercode_execpolicy::blocking_append_allow_prefix_rule;
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: hypercode_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(hypercode_home);
...
fn default_policy_path(hypercode_home: &Path) -> PathBuf {
    hypercode_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and hypercode prompts the user to run outside the sandbox
...
mod tests {
    ...
    fn config_stack_for_dot_hypercode_folder(dot_hypercode_folder: &Path) -> ConfigLayerStack {
        let dot_hypercode_folder = AbsolutePathBuf::from_absolute_path(dot_hypercode_folder)
            .expect("absolute dot_hypercode_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_hypercode_folder },
    ...
    async fn returns_empty_policy_when_feature_disabled() {
        ...
        let config_stack = config_stack_for_dot_hypercode_folder(temp_dir.path());
    ...
    async fn loads_policies_from_multiple_config_layers() -> anyhow::Result<()> {
        ...
        let project_dot_hypercode_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
        let layers = vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_hypercode_folder: project_dot_hypercode_folder,
                },
                TomlValue::Table(Default::default()),
            ),
        ];
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        ...
        manager
            .append_amendment_and_update(hypercode_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(hypercode_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(hypercode_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;
