...
pub fn apply_blocking(
    ...
    std::fs::create_dir_all(hypercode_home).with_context(|| {
        format!(
            "failed to create Hypercode home directory at {}",
            hypercode_home.display()
        )
    })?;

    let tmp = NamedTempFile::new_in(hypercode_home)?;
...
pub async fn apply(
    ...
    let hypercode_home = hypercode_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&hypercode_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(hypercode_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.hypercode_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let contents =
            std::fs::read_to_string(hypercode_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#,
        ...
        let raw = std::fs::read_to_string(hypercode_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
        std::fs::write(hypercode_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#;
    ...
    #[test]
    fn blocking_set_hide_gpt_5_1_hypercode_max_migration_prompt_preserves_table() {
        let tmp = tempdir().expect("tmpdir");
        let hypercode_home = tmp.path();
        std::fs::write(
            hypercode_home.join(CONFIG_TOML_FILE),
...
"#,
        ...
        apply_blocking(hypercode_home, None, &[ConfigEdit::ReplaceMcpServers(servers)])
            .expect("persist");
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !hypercode_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        contents = std::fs::read_to_string(hypercode_home.join(CONFIG_TOML_FILE)).expect("read config");
