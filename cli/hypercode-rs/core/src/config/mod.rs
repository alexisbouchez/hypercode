...
use hypercode_app_server_protocol::Tools;
use hypercode_app_server_protocol::UserSavedConfig;
use hypercode_protocol::config_types::AltScreenMode;
...
use hypercode_protocol::config_types::TrustLevel;
use hypercode_protocol::config_types::Verbosity;
...
use hypercode_utils_absolute_path::AbsolutePathBuf;
use hypercode_utils_absolute_path::AbsolutePathBufGuard;
use dirs::home_dir;
...
pub use hypercode_git::GhostSnapshotConfig;
...
pub(crate) fn test_config() -> Config {
    let hypercode_home = tempdir().expect("create temp dir");
    Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides::default(),
        hypercode_home.path().to_path_buf(),
    )
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Hypercode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Hypercode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.hypercode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Hypercode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Hypercode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Hypercode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Hypercode can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Hypercode unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/hypercode/blob/main/hypercode-rs/rmcp-client/src/oauth.rs#L2
    /// file: HYPERCODE_HOME/.credentials.json
    ///       This file will be readable to Hypercode and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Hypercode will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Hypercode state (defaults to `~/.hypercode` but can be
    /// overridden by the `HYPERCODE_HOME` environment variable).
    pub hypercode_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.hypercode/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `hypercode-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `hypercode-linux-sandbox`.
    pub hypercode_linux_sandbox_exe: Option<PathBuf>,
    ...
    /// When `true`, checks for Hypercode updates on startup and surfaces update prompts.
    /// Set to `false` only if your Hypercode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Hypercode product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Hypercode product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn hypercode_home(mut self, hypercode_home: PathBuf) -> Self {
        self.hypercode_home = Some(hypercode_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let hypercode_home = hypercode_home.map_or_else(find_hypercode_home, std::io::Result::Ok)?;
        ...
        let config_layer_stack =
            load_config_layers_state(&hypercode_home, Some(cwd), &cli_overrides, loader_overrides)
                .await?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let hypercode_home = find_hypercode_home()?;
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &hypercode_home)?;
    ...
    /// ignores user settings. For example, the `hypercode exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd` and `hypercode_linux_sandbox_exe`.
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, hypercode_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .hypercode/ folders.
    let cwd: Option<AbsolutePathBuf> = None;
    let config_layer_stack =
        load_config_layers_state(hypercode_home, cwd, &cli_overrides, LoaderOverrides::default())
            .await?;
    let merged_toml = config_layer_stack.effective_config();
...
/// Patch `HYPERCODE_HOME/config.toml` project state to set trust level.
/// Use with caution.
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(hypercode_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(hypercode_home: &Path, provider: &str) -> std::io::Result<()> {
    ...
    let config_path = hypercode_home.join(CONFIG_TOML_FILE);
...
/// Base config deserialized from ~/.hypercode/config.toml.
...
pub struct ConfigToml {
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/hypercode/blob/main/hypercode-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Hypercode home directory.
    ...
    /// directories for `.hypercode` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, hypercode_home, config_layer_stack)
    }
    ...
    fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&hypercode_home));
    ...
    fn load_instructions(hypercode_dir: Option<&Path>) -> Option<String> {
        let base = hypercode_dir?;
...
///   directory exists.
pub fn find_hypercode_home() -> std::io::Result<PathBuf> {
...
/// Returns the path to the folder where Hypercode logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.hypercode_home.clone();
    p.push("log");
    Ok(p)
}
...
"#,
    ...
    fn config_defaults_to_file_cli_auth_store_mode() -> std::io::Result<()> {
        let hypercode_home = TempDir::new()?;
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let hypercode_home = TempDir::new()?;
        let managed_path = hypercode_home.path().join("managed_config.toml");
        let config_path = hypercode_home.path().join(CONFIG_TOML_FILE);
        ...
        let cwd = AbsolutePathBuf::try_from(hypercode_home.path())?;
        let config_layer_stack =
            load_config_layers_state(hypercode_home.path(), Some(cwd), &Vec::new(), overrides).await?;
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(hypercode_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(hypercode_home.path()).await?;
    ...
    async fn managed_config_wins_over_cli_overrides() -> anyhow::Result<()> {
        ...
        std::fs::write(
            hypercode_home.path().join(CONFIG_TOML_FILE),
            "model = \"base\"\n",
        )?;
...
"#,
        ...
        let err = load_global_mcp_servers(hypercode_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/hypercode-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/hypercode-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/hypercode-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(hypercode_home.path())
            .set_model(Some("gpt-5.1-codex"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(hypercode_home.path().join(CONFIG_TOML_FILE)).await?;
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn hypercode_home(&self) -> PathBuf {
            self.hypercode_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let hypercode_home = TempDir::new()?;
        let workspace = hypercode_home.path().join("workspace");
...
"#;
        ...
        let hypercode_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            hypercode_home: hypercode_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                hypercode_home: fixture.hypercode_home(),
                config_layer_stack: Default::default(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                hypercode_linux_sandbox_exe: None,
...
projects = { "/Users/mbolin/code/hypercode4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/hypercode3" = { trust_level = "trusted" } }
model = "foo""#;
        let mut doc = initial.parse::<DocumentMut>()?;

        // Approve a new directory
        let new_project = Path::new("/Users/mbolin/code/hypercode2");
...
[projects."/Users/mbolin/code/hypercode4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/hypercode3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/hypercode2"]
trust_level = "trusted"
"#;
        assert_eq!(contents, expected);
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let hypercode_home = temp_dir.path();
        let config_path = hypercode_home.join(CONFIG_TOML_FILE);

        // Test setting valid provider on empty config
        set_default_oss_provider(hypercode_home, OLLAMA_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        assert!(content.contains("oss_provider = \"ollama\""));

        // Test updating existing config
        std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
        set_default_oss_provider(hypercode_home, LMSTUDIO_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        ...
        let result = set_default_oss_provider(hypercode_home, "invalid_provider");
