...
use hypercode_app_server_protocol::Config as ApiConfig;
use hypercode_app_server_protocol::ConfigBatchWriteParams;
use hypercode_app_server_protocol::ConfigLayerMetadata;
...
use hypercode_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(hypercode_home: PathBuf) -> Self {
        Self {
            hypercode_home,
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<hypercode_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.hypercode_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.hypercode_home)
    ...
    /// include any in-repo .hypercode/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_hypercode_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_hypercode_folder.display(),
        ),
...
"#;
    ...
    async fn write_value_preserves_comments_and_order() -> Result<()> {
        ...
        let original = r#"# Hypercode user configuration
model = "gpt-5"
...
"#;
        ...
        let expected = r#"# Hypercode user configuration
model = "gpt-5"
approval_policy = "on-request"
