...
impl ShellSnapshot {
    pub async fn try_new(hypercode_home: &Path, session_id: ThreadId, shell: &Shell) -> Option<Self> {
        // File to store the snapshot
        let extension = match shell.shell_type {
            ShellType::PowerShell => "ps1",
            _ => "sh",
        };
        let path = hypercode_home
            .join(SNAPSHOT_DIR)
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&hypercode_home, cleanup_session_id).await {
...
pub async fn cleanup_stale_snapshots(hypercode_home: &Path, active_session_id: ThreadId) -> Result<()> {
    let snapshot_dir = hypercode_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(hypercode_home, session_id).await?;
...
mod tests {
    ...
    async fn write_rollout_stub(hypercode_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
        let dir = hypercode_home
            .join("sessions")
            .join("2025")
            .join("01")
            .join("01");
    ...
    async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
        ...
        write_rollout_stub(hypercode_home, live_session).await?;
        fs::write(&live_snapshot, "live").await?;
        fs::write(&orphan_snapshot, "orphan").await?;
        fs::write(&invalid_snapshot, "invalid").await?;

        cleanup_stale_snapshots(hypercode_home, ThreadId::new()).await?;
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(hypercode_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(hypercode_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(hypercode_home, active_session).await?;
