...
impl SkillsManager {
    pub fn new(hypercode_home: PathBuf) -> Self {
        if let Err(err) = install_system_skills(&hypercode_home) {
...
mod tests {
    ...
    fn write_user_skill(hypercode_home: &TempDir, dir: &str, name: &str, description: &str) {
        let skill_dir = hypercode_home.path().join("skills").join(dir);
    ...
    async fn skills_for_config_seeds_cache_by_cwd() {
        ...
        let skills_manager = SkillsManager::new(hypercode_home.path().to_path_buf());

        write_user_skill(&hypercode_home, "a", "skill-a", "from a");
        ...
        write_user_skill(&hypercode_home, "b", "skill-b", "from b");
