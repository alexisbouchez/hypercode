...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".hypercode-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `HYPERCODE_HOME/skills/.system`.
pub(crate) fn system_cache_root_dir(hypercode_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(hypercode_home)
        .and_then(|hypercode_home| system_cache_root_dir_abs(&hypercode_home))
...
fn system_cache_root_dir_abs(hypercode_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    hypercode_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `HYPERCODE_HOME/skills/.system`.
///
...
/// install is skipped.
pub(crate) fn install_system_skills(hypercode_home: &Path) -> Result<(), SystemSkillsError> {
    let hypercode_home = AbsolutePathBuf::try_from(hypercode_home)
        .map_err(|source| SystemSkillsError::io("normalize hypercode home dir", source))?;
    let skills_root_dir = hypercode_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&hypercode_home)
