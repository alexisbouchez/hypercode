...
use hypercode_app_server_protocol::ConfigLayerSource;
use hypercode_protocol::protocol::SkillScope;
    ...
    for layer in config_layer_stack.layers_high_to_low() {
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // `$HYPERCODE_HOME/skills` (user-installed skills).
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                });

                // Embedded system skills are cached under `$HYPERCODE_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/hypercode/` on Unix, so treat
                // `/etc/hypercode/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinks for user, admin, and repo skills. System skills are written by Hypercode itself.
...
mod tests {
    ...
    const REPO_ROOT_CONFIG_DIR_NAME: &str = ".hypercode";

    async fn make_config(hypercode_home: &TempDir) -> Config {
        make_config_for_cwd(hypercode_home, hypercode_home.path().to_path_buf()).await
    }

    async fn make_config_for_cwd(hypercode_home: &TempDir, cwd: PathBuf) -> Config {
    ...
    -> anyhow::Result<()> {
        ...
        let system_folder = tmp.path().join("etc/hypercode");
        let user_folder = tmp.path().join("home/hypercode");
    ...
    fn write_skill(hypercode_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&hypercode_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &hypercode_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        let shared = tempfile::tempdir().expect("tempdir");

        let shared_skill_path = write_skill_at(shared.path(), "demo", "linked-skill", "from link");

        fs::create_dir_all(hypercode_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &hypercode_home.path().join("skills/shared"));

        let cfg = make_config(&hypercode_home).await;
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        // Create a cycle:
        //   $HYPERCODE_HOME/skills/cycle/loop -> $HYPERCODE_HOME/skills/cycle
        let cycle_dir = hypercode_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&hypercode_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = hypercode_home.path().join("skills/.system");
    ...
    async fn loads_valid_skill() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&hypercode_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&hypercode_home).await;
    ...
    async fn skips_hidden_and_invalid() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = hypercode_home.path().join("skills/.hidden");
        ...
        let invalid_dir = hypercode_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&hypercode_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&hypercode_home).await;
        ...
        write_skill(&hypercode_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    #[tokio::test]
    async fn loads_skills_from_all_hypercode_dirs_under_project_root() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
        ...
        let cfg = make_config_for_cwd(&hypercode_home, nested_dir).await;
    ...
    #[tokio::test]
    async fn loads_skills_from_hypercode_dir_when_not_git_repo() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        ...
        let cfg = make_config_for_cwd(&hypercode_home, work_dir.path().to_path_buf()).await;
    ...
    async fn deduplicates_by_name_preferring_repo_over_user() {
        ...
        let _user_skill_path = write_skill(&hypercode_home, "user", "dupe-skill", "from user");
    ...
    async fn loads_system_skills_when_present() {
        ...
        let _system_skill_path =
            write_system_skill(&hypercode_home, "system", "dupe-skill", "from system");
        let user_skill_path = write_skill(&hypercode_home, "user", "dupe-skill", "from user");
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&hypercode_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&hypercode_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&hypercode_home, "system", "system-skill", "from system");
    ...
    #[tokio::test]
    async fn deduplicates_by_name_preferring_nearest_project_hypercode_dir() {
        let hypercode_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
