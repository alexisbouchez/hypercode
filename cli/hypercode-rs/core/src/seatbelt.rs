...
use crate::spawn::HYPERCODE_SANDBOX_ENV_VAR;
...
pub async fn spawn_command_under_seatbelt(
    ...
    let arg0 = None;
    env.insert(HYPERCODE_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
    spawn_child_async(
...
mod tests {
    ...
    #[test]
    fn create_seatbelt_args_with_read_only_git_and_hypercode_subpaths() {
        // Create a temporary workspace with two writable roots: one containing
        // top-level .git and .hypercode directories and one without them.
        let tmp = TempDir::new().expect("tempdir");
        let PopulatedTmp {
            vulnerable_root,
            vulnerable_root_canonical,
            dot_git_canonical,
            dot_hypercode_canonical,
            empty_root,
        ...
        // write to .hypercode/config.toml in the vulnerable root.
        ...
        // - write access to WRITABLE_ROOT_0 (but not its .git or .hypercode), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
        ...
        let mut expected_args = vec![
            ...
            format!(
                ...
                dot_hypercode_canonical.to_string_lossy()
            ),
            format!(
                "-DWRITABLE_ROOT_1={}",
        ...
        // Verify that .hypercode/config.toml cannot be modified under the generated
        // Seatbelt policy.
        let config_toml = dot_hypercode_canonical.join("config.toml");
        ...
        // Verify that writing a file to the folder containing .git and .hypercode is allowed.
    ...
    fn create_seatbelt_args_for_cwd_as_git_repo() {
        ...
        // `.hypercode` checks are done properly for cwd.
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: false,
...
"#,
    ...
    struct PopulatedTmp {
        /// Path containing a .git and .hypercode subfolder.
        ...
        /// ran `git commit` themselves, or modified .hypercode/config.toml to
        ...
        dot_hypercode_canonical: PathBuf,

        /// Path without .git or .hypercode subfolders.
        empty_root: PathBuf,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".hypercode")).expect("create .hypercode");
        fs::write(
            vulnerable_root.join(".hypercode").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        .expect("write .hypercode/config.toml");
        ...
        let dot_hypercode_canonical = vulnerable_root_canonical.join(".hypercode");
