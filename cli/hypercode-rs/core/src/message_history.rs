...
//!
//! The history is stored at `~/.hypercode/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.hypercode`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.hypercode_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.hypercode/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        let hypercode_home = TempDir::new().expect("create temp dir");
        ...
        let history_path = hypercode_home.path().join("history.jsonl");
