...
use hypercode_client::HypercodeHttpClient;
use hypercode_protocol::account::PlanType as AccountPlanType;
use serde_json::Value;
use thiserror::Error;

#[derive(Debug, Clone)]
pub struct HypercodeAuth {
    pub mode: AuthMode,

    pub(crate) api_key: Option<String>,
    pub(crate) auth_dot_json: Arc<Mutex<Option<AuthDotJson>>>,
    storage: Arc<dyn AuthStorageBackend>,
    pub(crate) client: HypercodeHttpClient,
}

impl PartialEq for HypercodeAuth {
...
impl HypercodeAuth {
    ...
    ) -> std::io::Result<Option<HypercodeAuth>> {
        load_auth(hypercode_home, false, auth_credentials_store_mode)
    }
    ...
    pub fn get_token(&self) -> Result<String, std::io::Error> {
        match self.mode {
            AuthMode::ApiKey | AuthMode::Mistral => Ok(self.api_key.clone().unwrap_or_default()),
            AuthMode::ChatGPT => {
    ...
    fn from_api_key_with_client(api_key: &str, client: HypercodeHttpClient) -> Self {
        Self {
    ...
    fn from_mistral_api_key_with_client(api_key: &str, client: HypercodeHttpClient) -> Self {
        Self {
            api_key: Some(api_key.to_owned()),
            mode: AuthMode::Mistral,
            storage: create_auth_storage(PathBuf::new(), AuthCredentialsStoreMode::File),
            auth_dot_json: Arc::new(Mutex::new(None)),
            client,
        }
    ...
    pub fn from_mistral_api_key(api_key: &str) -> Self {
        Self::from_mistral_api_key_with_client(api_key, crate::default_client::create_client())
    }
}
...
pub fn read_hypercode_api_key_from_env() -> Option<String> {
...
pub fn read_mistral_api_key_from_env() -> Option<String> {
    env::var(MISTRAL_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `hypercode_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    hypercode_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(hypercode_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}

/// Writes an `auth.json` that contains only the OpenAI API key.
pub fn login_with_api_key(
    ...
    save_auth(hypercode_home, &auth_dot_json, auth_credentials_store_mode)
}

/// Writes an `auth.json` that contains only the Mistral API key.
pub fn login_with_mistral_api_key(
    hypercode_home: &Path,
    api_key: &str,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<()> {
    let auth_dot_json = AuthDotJson {
        openai_api_key: None,
        mistral_api_key: Some(api_key.to_string()),
        tokens: None,
        last_refresh: None,
    };
    save_auth(hypercode_home, &auth_dot_json, auth_credentials_store_mode)
...
pub fn enforce_login_restrictions(config: &Config) -> std::io::Result<()> {
    ...
    if let Some(required_method) = config.forced_login_method {
        let method_violation = match (required_method, auth.mode) {
            (ForcedLoginMethod::Api, AuthMode::ApiKey | AuthMode::Mistral) => None,
            (ForcedLoginMethod::Chatgpt, AuthMode::ChatGPT) => None,
            (ForcedLoginMethod::Api, AuthMode::ChatGPT) => Some(
                "API key login is required, but ChatGPT is currently being used. Logging out."
                    .to_string(),
            ),
            (ForcedLoginMethod::Chatgpt, AuthMode::ApiKey | AuthMode::Mistral) => Some(
...
fn logout_with_message(
    ...
    match logout(hypercode_home, auth_credentials_store_mode) {
...
fn load_auth(
    hypercode_home: &Path,
    enable_hypercode_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<HypercodeAuth>> {
    if enable_hypercode_api_key_env && let Some(api_key) = read_hypercode_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(HypercodeAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
    ...
    let AuthDotJson {
        ...
        mistral_api_key: auth_json_mistral_api_key,
        tokens,
        last_refresh,
    } = auth_dot_json;

    // Prefer AuthMode.ApiKey if OpenAI API key is set in the auth.json.
    if let Some(api_key) = &auth_json_api_key {
        return Ok(Some(HypercodeAuth::from_api_key_with_client(api_key, client)));
    }

    // Use Mistral mode if Mistral API key is set in the auth.json.
    if let Some(api_key) = &auth_json_mistral_api_key {
        return Ok(Some(HypercodeAuth::from_mistral_api_key_with_client(api_key, client)));
    }

    Ok(Some(HypercodeAuth {
        api_key: None,
        mode: AuthMode::ChatGPT,
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        let expected_account_id = manager
            .auth_cached()
            .as_ref()
            .and_then(HypercodeAuth::get_account_id);
        Self {
            manager,
...
/// hands out cloned `HypercodeAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific HypercodeAuth, for testing only.
    pub fn from_auth_for_testing(auth: HypercodeAuth) -> Arc<Self> {
        let cached = CachedAuth { auth: Some(auth) };

        Arc::new(Self {
            hypercode_home: PathBuf::from("non-existent"),
            inner: RwLock::new(cached),
            enable_hypercode_api_key_env: false,
            auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        })
    }

    #[cfg(any(test, feature = "test-support"))]
    /// Create an AuthManager with a specific HypercodeAuth and hypercode home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: HypercodeAuth, hypercode_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth { auth: Some(auth) };
        Arc::new(Self {
            hypercode_home,
            inner: RwLock::new(cached),
    ...
    pub fn auth_cached(&self) -> Option<HypercodeAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    /// Current cached auth (clone). May be `None` if not logged in or load failed.
    /// Refreshes cached ChatGPT tokens if they are stale before returning.
    pub async fn auth(&self) -> Option<HypercodeAuth> {
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(HypercodeAuth::get_account_id);
    ...
    fn auths_equal(a: &Option<HypercodeAuth>, b: &Option<HypercodeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<HypercodeAuth> {
        load_auth(
            &self.hypercode_home,
            self.enable_hypercode_api_key_env,
    ...
    fn set_auth(&self, new_auth: Option<HypercodeAuth>) -> bool {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout(&self.hypercode_home, self.auth_credentials_store_mode)?;
    ...
    async fn refresh_if_stale(&self, auth: &HypercodeAuth) -> Result<bool, RefreshTokenError> {
        if auth.mode != AuthMode::ChatGPT {
            return Ok(false);
        }
...
mod tests {
    ...
    async fn refresh_without_id_token() {
        let hypercode_home = tempdir().unwrap();
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = HypercodeAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
            .expect("call should succeed");
        assert_eq!(auth, None);
    }

    #[tokio::test]
    #[serial(hypercode_api_key)]
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let HypercodeAuth {
            ...
            storage: _,
            ..
        } = super::load_auth(hypercode_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, hypercode_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(hypercode_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        let hypercode_home = tempdir().unwrap();
        login_with_api_key(hypercode_home.path(), "sk-test", AuthCredentialsStoreMode::File)
            .expect("seed api key");

        let config = build_config(hypercode_home.path(), Some(ForcedLoginMethod::Chatgpt), None).await;

        let err = super::enforce_login_restrictions(&config)
            .expect_err("expected method mismatch to error");
        assert!(err.to_string().contains("ChatGPT login is required"));
        assert!(
            !hypercode_home.path().join("auth.json").exists(),
    ...
    async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
        ...
        let config = build_config(hypercode_home.path(), None, Some("org_mine".to_string())).await;
    ...
    fn plan_type_maps_known_plan() {
        ...
        let auth = super::load_auth(hypercode_home.path(), false, AuthCredentialsStoreMode::File)
            .expect("load auth")
