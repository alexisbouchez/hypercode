...
use crate::spawn::HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use crate::tools::sandboxing::SandboxablePreference;
pub use hypercode_protocol::models::SandboxPermissions;
...
pub(crate) enum SandboxTransformError {
    #[error("missing hypercode-linux-sandbox executable path")]
...
impl SandboxManager {
    ...
    pub(crate) fn transform(
        ...
        if !policy.has_full_network_access() {
            env.insert(
                HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        ...
        let (command, sandbox_env, arg0_override) = match sandbox {
            ...
            SandboxType::MacosSeatbelt => {
                ...
                seatbelt_env.insert(HYPERCODE_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
                let mut args =
            ...
            SandboxType::LinuxSeccomp => {
                let exe = hypercode_linux_sandbox_exe
                ...
                (
                    full_command,
                    HashMap::new(),
                    Some("hypercode-linux-sandbox".to_string()),
                )
            }
            // On Windows, the restricted token sandbox executes in-process via the
            // hypercode-windows-sandbox crate. We leave the command unchanged here and
