...
//!
//! The [`McpConnectionManager`] owns one [`hypercode_rmcp_client::RmcpClient`] per
...
use hypercode_protocol::protocol::McpStartupCompleteEvent;
use hypercode_protocol::protocol::McpStartupFailure;
use hypercode_protocol::protocol::McpStartupStatus;
use hypercode_protocol::protocol::McpStartupUpdateEvent;
...
use hypercode_rmcp_client::RmcpClient;
use hypercode_rmcp_client::SendElicitation;
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "hypercode/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "hypercode/sandbox-state/update";
...
async fn start_server_task(
    ...
    let params = mcp_types::InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "hypercode-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Hypercode".into()),
            // This field is used by Hypercode when it is an MCP
            // server: it should not be used when Hypercode is
            // an MCP client.
            user_agent: None,
        },
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!(
            "The {server_name} MCP server is not logged in. Run `hypercode mcp login {server_name}`."
        )
...
mod tests {
    use super::*;
    use hypercode_protocol::protocol::McpAuthStatus;
