...
mod tests {
    ...
    async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            "export CODEX_INTERACTIVE_SHELL_VAR=hypercode\n",
            2_500,
        )
        .await?;
        ...
        .await?;
        assert!(
            out_2.output.contains("hypercode"),
    ...
    async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
        ...
        .await?;
        assert!(
            out_3.output.contains("hypercode"),
    ...
    async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
        ...
        let result = exec_command(&session, &turn, "echo hypercode", 120_000).await?;

        assert!(result.process_id.is_some());
        assert!(result.output.contains("hypercode"));
    ...
    async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
        let (session, turn) = test_session_and_turn().await;
        let result = exec_command(&session, &turn, "echo hypercode", 2_500).await?;
