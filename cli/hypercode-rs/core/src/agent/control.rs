...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> HypercodeThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn send_prompt(
        ...
        let result = state
            ...
            .await;
        if matches!(result, Err(HypercodeErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_agent(&self, agent_id: ThreadId) -> HypercodeResult<String> {
        let state = self.upgrade()?;
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> HypercodeResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
        Ok(thread.subscribe_status())
    }

    fn upgrade(&self) -> HypercodeResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| HypercodeErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
}
...
mod tests {
    ...
    use hypercode_protocol::protocol::TurnCompleteEvent;
    ...
    async fn test_config() -> (TempDir, Config) {
        ...
        let config = ConfigBuilder::default()
            .hypercode_home(home.path().to_path_buf())
            .build()
            .await
    ...
    impl AgentControlHarness {
        ...
        async fn start_thread(&self) -> (ThreadId, Arc<HypercodeThread>) {
            let new_thread = self
                .manager
    ...
    async fn send_prompt_errors_when_thread_missing() {
        ...
        assert_matches!(err, HypercodeErr::ThreadNotFound(id) if id == thread_id);
    }
