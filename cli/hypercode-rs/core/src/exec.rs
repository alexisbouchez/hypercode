...
use hypercode_utils_pty::process_group::kill_child_process_group;
...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            hypercode_linux_sandbox_exe.as_ref(),
        )
        .map_err(HypercodeErr::from)?;
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_hypercode_home;
    use crate::safety::is_windows_elevated_sandbox_enabled;
    use hypercode_windows_sandbox::run_windows_sandbox_capture;
    use hypercode_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        HypercodeErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let hypercode_home = find_hypercode_home().map_err(|err| {
        HypercodeErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve hypercode_home: {err}"
        )))
    })?;
    ...
    let capture = match spawn_res {
        Ok(Ok(v)) => v,
        Ok(Err(err)) => {
            return Err(HypercodeErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, HypercodeErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(HypercodeErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(HypercodeErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(HypercodeErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    use super::HypercodeErr;
    use crate::sandboxing::SandboxTransformError;

    impl From<SandboxTransformError> for HypercodeErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                ...
                SandboxTransformError::SeatbeltUnavailable => HypercodeErr::UnsupportedOperation(
...
async fn consume_truncated_output(
    ...
    let stdout_reader = child.stdout.take().ok_or_else(|| {
        HypercodeErr::Io(io::Error::other(
...
mod tests {
    ...
    async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
        ...
        let output = match result {
            Err(HypercodeErr::Sandbox(SandboxErr::Timeout { output })) => output,
