...
use chrono::Local;
use chrono::Utc;
use hypercode_async_utils::CancelErr;
...
use hypercode_protocol::protocol::RateLimitSnapshot;
...
pub type Result<T> = std::result::Result<T, HypercodeErr>;
...
pub enum HypercodeErr {
    ...
    #[error(
        "Hypercode ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `HypercodeError::Spawn` variant.
    ...
    #[error(
        "To use Hypercode with your ChatGPT plan, upgrade to Plus: https://openai.com/chatgpt/pricing."
    )]
    UsageNotIncluded,
    ...
    #[error("hypercode-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for HypercodeErr {
    fn from(_: CancelErr) -> Self {
        HypercodeErr::TurnAborted
    }
}
...
impl HypercodeErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            HypercodeErr::TurnAborted
            | HypercodeErr::Interrupted
            | HypercodeErr::EnvVar(_)
            | HypercodeErr::Fatal(_)
            | HypercodeErr::UsageNotIncluded
            | HypercodeErr::QuotaExceeded
            | HypercodeErr::InvalidImageRequest()
            | HypercodeErr::InvalidRequest(_)
            | HypercodeErr::RefreshTokenFailed(_)
            | HypercodeErr::UnsupportedOperation(_)
            | HypercodeErr::Sandbox(_)
            | HypercodeErr::LandlockSandboxExecutableNotProvided
            | HypercodeErr::RetryLimit(_)
            | HypercodeErr::ContextWindowExceeded
            | HypercodeErr::ThreadNotFound(_)
            | HypercodeErr::Spawn
            | HypercodeErr::SessionConfiguredNotFirstEvent
            | HypercodeErr::UsageLimitReached(_) => false,
            HypercodeErr::Stream(..)
            | HypercodeErr::Timeout
            | HypercodeErr::UnexpectedStatus(_)
            | HypercodeErr::ResponseStreamFailed(_)
            | HypercodeErr::ConnectionFailed(_)
            | HypercodeErr::InternalServerError
            | HypercodeErr::InternalAgentDied
            | HypercodeErr::Io(_)
            | HypercodeErr::Json(_)
            | HypercodeErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            HypercodeErr::LandlockRuleset(_) | HypercodeErr::LandlockPathFd(_) => false,
        }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/hypercode/settings/usage to purchase more credits{}",
            ...
            Some(PlanType::Known(KnownPlan::Free)) => {
                "You've hit your usage limit. Upgrade to Plus to continue using Hypercode (https://openai.com/chatgpt/pricing)."
                    .to_string()
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/hypercode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl HypercodeErr {
    /// Minimal shim so that existing `e.downcast_ref::<HypercodeErr>()` checks continue to compile
    ...
    pub fn to_hypercode_protocol_error(&self) -> HypercodeErrorInfo {
        match self {
            HypercodeErr::ContextWindowExceeded => HypercodeErrorInfo::ContextWindowExceeded,
            HypercodeErr::UsageLimitReached(_)
            | HypercodeErr::QuotaExceeded
            | HypercodeErr::UsageNotIncluded => HypercodeErrorInfo::UsageLimitExceeded,
            HypercodeErr::RetryLimit(_) => HypercodeErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            HypercodeErr::ConnectionFailed(_) => HypercodeErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            HypercodeErr::ResponseStreamFailed(_) => HypercodeErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            HypercodeErr::RefreshTokenFailed(_) => HypercodeErrorInfo::Unauthorized,
            HypercodeErr::SessionConfiguredNotFirstEvent
            | HypercodeErr::InternalServerError
            | HypercodeErr::InternalAgentDied => HypercodeErrorInfo::InternalServerError,
            HypercodeErr::UnsupportedOperation(_) | HypercodeErr::ThreadNotFound(_) => {
                HypercodeErrorInfo::BadRequest
            }
            HypercodeErr::Sandbox(_) => HypercodeErrorInfo::SandboxError,
            _ => HypercodeErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            hypercode_error_info: Some(self.to_hypercode_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            HypercodeErr::RetryLimit(err) => Some(err.status),
            HypercodeErr::UnexpectedStatus(err) => Some(err.status),
            HypercodeErr::ConnectionFailed(err) => err.source.status(),
            HypercodeErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &HypercodeErr) -> String {
    let message = match e {
        HypercodeErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        HypercodeErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    use chrono::TimeZone;
    use chrono::Utc;
    use hypercode_protocol::protocol::RateLimitWindow;
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/hypercode/settings/usage to purchase more credits or try again later."
        );
    }
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = HypercodeErr::Sandbox(SandboxErr::Denied {
    ...
    fn to_error_event_handles_response_stream_failed() {
        ...
        let err = HypercodeErr::ResponseStreamFailed(ResponseStreamFailed {
            source,
        ...
        assert_eq!(
            event.hypercode_error_info,
            Some(HypercodeErrorInfo::ResponseStreamConnectionFailed {
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit https://chatgpt.com/hypercode/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/hypercode/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
