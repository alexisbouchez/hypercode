...
use hypercode_apply_patch::ApplyPatchAction;
use hypercode_apply_patch::ApplyPatchFileChange;
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        match hypercode_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            hypercode_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                match apply_patch::apply_patch(turn.as_ref(), changes).await {
                    ...
                    InternalApplyPatchInvocation::DelegateToExec(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            ...
                            timeout_ms: None,
                            hypercode_exe: turn.hypercode_linux_sandbox_exe.clone(),
                        };
            ...
            hypercode_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            hypercode_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            hypercode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match hypercode_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        hypercode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
...
mod tests {
    use super::*;
    use hypercode_apply_patch::MaybeApplyPatchVerified;
...
*** End Patch"#;
        ...
        let action = match hypercode_apply_patch::maybe_parse_apply_patch_verified(&argv, cwd) {
