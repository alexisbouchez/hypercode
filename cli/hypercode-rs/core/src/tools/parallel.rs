...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, HypercodeErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(HypercodeErr::Fatal(message)),
                Ok(Err(other)) => Err(HypercodeErr::Fatal(other.to_string())),
                Err(err) => Err(HypercodeErr::Fatal(format!(
