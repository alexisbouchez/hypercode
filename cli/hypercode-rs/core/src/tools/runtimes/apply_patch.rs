...
//! `hypercode --hypercode-run-as-apply-patch`, and runs under the current
...
pub struct ApplyPatchRequest {
    ...
    pub hypercode_exe: Option<PathBuf>,
}
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(req: &ApplyPatchRequest) -> Result<CommandSpec, ToolError> {
        use std::env;
        let exe = if let Some(path) = &req.hypercode_exe {
            path.clone()
        } else {
            env::current_exe()
                .map_err(|e| ToolError::Rejected(format!("failed to determine hypercode exe: {e}")))?
        };
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let env = attempt
            .env_for(spec)
            .map_err(|err| ToolError::Hypercode(err.into()))?;
        let out = execute_env(env, attempt.policy, Self::stdout_stream(ctx))
            .await
            .map_err(ToolError::Hypercode)?;
        Ok(out)
    }
}
