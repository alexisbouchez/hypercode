...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when HYPERCODE_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".hypercode")).await?;
    tokio::fs::create_dir_all(project_root.join(".hypercode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".hypercode").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".hypercode").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;

    let hypercode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&hypercode_home).await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_hypercode_folder } => Some(dot_hypercode_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".hypercode").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".hypercode").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_hypercode_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".hypercode").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".hypercode").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".hypercode").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".hypercode").join("child.txt"),
        "child instructions",
    )
    .await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_hypercode_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_hypercode_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".hypercode"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
        }],
...
async fn project_root_markers_supports_alternate_markers() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&hypercode_home).await?;
    tokio::fs::write(
        hypercode_home.join(CONFIG_TOML_FILE),
        r#"
