...
use hypercode_app_server_protocol::ConfigLayer;
...
impl ConfigLayerEntry {
    ...
    // Get the `.hypercode/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::Mdm { .. } => None,
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file } => file.parent(),
            ConfigLayerSource::Project { dot_hypercode_folder } => Some(dot_hypercode_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_hypercode_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_hypercode_folder: current_project_dot_hypercode_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_hypercode_folder {
                ...
                if previous == current_project_dot_hypercode_folder
                    || !current_project_dot_hypercode_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_hypercode_folder = Some(current_project_dot_hypercode_folder);
        }
    }
