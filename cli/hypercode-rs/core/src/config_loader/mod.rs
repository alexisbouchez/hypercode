...
const DEFAULT_REQUIREMENTS_TOML_FILE_UNIX: &str = "/etc/hypercode/requirements.toml";

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/hypercode/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/hypercode/config.toml";
...
/// - system    `/etc/hypercode/requirements.toml`
///
/// For backwards compatibility, we also load from
/// `/etc/hypercode/managed_config.toml` and map it to
/// `/etc/hypercode/requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - admin:    managed preferences (*)
/// - system    `/etc/hypercode/config.toml`
/// - user      `${HYPERCODE_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml`
/// - tree      parent directories up to root looking for `./.hypercode/config.toml`
/// - repo      `$(git rev-parse --show-toplevel)/.hypercode/config.toml`
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/hypercode/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    // Honor /etc/hypercode/requirements.toml.
    if cfg!(unix) {
    ...
    let loaded_config_layers = layer_io::load_config_layers_internal(hypercode_home, overrides).await?;
    ...
    // Add a layer for $HYPERCODE_HOME/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, hypercode_home)?;
...
/// If available, apply requirements from `/etc/hypercode/requirements.toml` to
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_hypercode = dir.join(".hypercode");
        if !tokio::fs::metadata(&dot_hypercode)
            .await
        ...
        let dot_hypercode_abs = AbsolutePathBuf::from_absolute_path(&dot_hypercode)?;
        let config_file = dot_hypercode_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_hypercode_abs.as_path())?;
                layers.push(ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_hypercode_folder: dot_hypercode_abs,
                    },
                    config,
                ));
            }
...
/// from a file like `/etc/hypercode/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Hypercode to function correctly.
