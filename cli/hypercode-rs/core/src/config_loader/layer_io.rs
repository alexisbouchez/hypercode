...
#[cfg(unix)]
const CODEX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/hypercode/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/hypercode/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(hypercode_home)),
    )?;
...
pub(super) fn managed_config_default_path(hypercode_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = hypercode_home;
        PathBuf::from(CODEX_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        hypercode_home.join("managed_config.toml")
    }
}
