# `hypercode-core` config loader

This module is the canonical place to **load and describe Hypercode configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `hypercode_core::config_loader`:

- `load_config_layers_state(hypercode_home, cwd_opt, cli_overrides, overrides) -> ConfigLayerStack`
- `ConfigLayerStack`
...
```rust
use hypercode_core::config_loader::{load_config_layers_state, LoaderOverrides};
