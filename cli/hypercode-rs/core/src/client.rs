...
use hypercode_api::AggregateStreamExt;
use hypercode_api::ChatClient as ApiChatClient;
use hypercode_api::CompactClient as ApiCompactClient;
use hypercode_api::CompactionInput as ApiCompactionInput;
use hypercode_api::Prompt as ApiPrompt;
use hypercode_api::RequestTelemetry;
use hypercode_api::ReqwestTransport;
use hypercode_api::ResponseAppendWsRequest;
use hypercode_api::ResponseCreateWsRequest;
use hypercode_api::ResponseStream as ApiResponseStream;
use hypercode_api::ResponsesClient as ApiResponsesClient;
use hypercode_api::ResponsesOptions as ApiResponsesOptions;
use hypercode_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use hypercode_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use hypercode_api::SseTelemetry;
use hypercode_api::TransportError;
use hypercode_api::build_conversation_headers;
use hypercode_api::common::Reasoning;
use hypercode_api::common::ResponsesWsRequest;
use hypercode_api::create_text_param_for_request;
use hypercode_api::error::ApiError;
use hypercode_api::requests::responses::Compression;
...
impl ModelClientSession {
    ...
    pub async fn stream(&mut self, prompt: &Prompt) -> Result<ResponseStream> {
        // Check auth mode to determine wire API - Mistral only supports Chat API
        let auth_mode = match &self.state.auth_manager {
            Some(manager) => manager.auth().await.map(|a| a.mode),
            None => None,
        };

        // Override wire_api for Mistral since it only supports Chat API
        // Check both auth_mode and provider configuration (via base_url or name)
        let is_mistral = matches!(auth_mode, Some(AuthMode::Mistral))
            || self.state.provider.is_mistral_provider();
        let effective_wire_api = if is_mistral {
            WireApi::Chat
        } else {
            self.state.provider.wire_api
        };

        match effective_wire_api {
    ...
    async fn websocket_connection(
        &mut self,
        api_provider: hypercode_api::Provider,
    ...
    fn responses_request_compression(&self, auth: Option<&crate::auth::HypercodeAuth>) -> Compression {
        if self
            .state
            .config
            .features
    ...
    async fn stream_chat_completions(&self, prompt: &Prompt) -> Result<ApiResponseStream> {
        if prompt.output_schema.is_some() {
            return Err(HypercodeErr::UnsupportedOperation(
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*CODEX_RS_SSE_FIXTURE {
            ...
            let stream =
                hypercode_api::stream_from_fixture(path, self.state.provider.stream_idle_timeout())
                    .map_err(map_api_error)?;
...
/// Adapts the core `Prompt` type into the `hypercode-api` payload shape.
...
fn beta_feature_headers(config: &Config) -> ApiHeaderMap {
    ...
    {
        headers.insert("x-hypercode-beta-features", header_value);
    }
    headers
}
...
/// the mapped `HypercodeErr` is returned to the caller.
async fn handle_unauthorized(
        ...
        && recovery.has_next()
    {
        return match recovery.next().await {
            Ok(_) => Ok(()),
            Err(RefreshTokenError::Permanent(failed)) => Err(HypercodeErr::RefreshTokenFailed(failed)),
            Err(RefreshTokenError::Transient(other)) => Err(HypercodeErr::Io(other)),
        };
    }
...
fn map_unauthorized_status(status: StatusCode) -> HypercodeErr {
