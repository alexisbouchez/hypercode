use hypercode_api::ModelsClient;
...
const MISTRAL_DEFAULT_MODEL: &str = "devstral-small-latest";
const CODEX_AUTO_BALANCED_MODEL: &str = "hypercode-auto-balanced";
...
impl ModelsManager {
    ...
    ///
    /// Uses `hypercode_home` to store cached model metadata and initializes with built-in presets.
    pub fn new(hypercode_home: PathBuf, auth_manager: Arc<AuthManager>) -> Self {
        let cache_path = hypercode_home.join(MODEL_CACHE_FILE);
    ...
    /// auth mode and available models (prefers `hypercode-auto-balanced` for ChatGPT auth).
    pub async fn get_default_model(
        ...
        // if hypercode-auto-balanced exists & signed in with chatgpt mode, return it, otherwise return the default model
        let auth_mode = self.auth_manager.get_auth_mode();
        let remote_models = self.get_remote_models(config).await;
        if auth_mode == Some(AuthMode::Mistral) {
            return MISTRAL_DEFAULT_MODEL.to_string();
        }
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        ...
        .await
        .map_err(|_| HypercodeErr::Timeout)?
...
mod tests {
    ...
    async fn refresh_available_models_sorts_and_marks_default() {
        ...
        let hypercode_home = tempdir().expect("temp dir");
        ...
        let manager =
            ModelsManager::with_provider(hypercode_home.path().to_path_buf(), auth_manager, provider);
