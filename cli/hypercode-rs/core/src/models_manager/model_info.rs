...
use hypercode_protocol::openai_models::ApplyPatchToolType;
...
use hypercode_protocol::openai_models::TruncationMode;
...
const GPT_5_CODEX_INSTRUCTIONS: &str = include_str!("../../gpt_5_hypercode_prompt.md");
...
pub(crate) fn find_model_info_for_slug(slug: &str) -> ModelInfo {
    ...
    } else if slug.starts_with("hypercode-mini-latest") {
        model_info!(
            slug,
            base_instructions: BASE_INSTRUCTIONS_WITH_APPLY_PATCH.to_string(),
    ...
    } else if slug.starts_with("exp-hypercode") || slug.starts_with("hypercode-1p") {
        model_info!(
            slug,
    ...
    } else if (slug.starts_with("gpt-5-hypercode")
        || slug.starts_with("gpt-5.1-codex")
        || slug.starts_with("hypercode-"))
        && !slug.contains("-mini")
    {
        model_info!(
            slug,
        ...
        && !slug.contains("hypercode")
    {
        model_info!(
            ...
            supports_parallel_tool_calls: true,
            context_window: Some(CONTEXT_WINDOW_272K),
            supported_reasoning_levels: supported_reasoning_level_low_medium_high_xhigh_non_hypercode(),
        )
    } else if slug.starts_with("gpt-5.1") && !slug.contains("hypercode") {
        model_info!(
            slug,
            apply_patch_tool_type: Some(ApplyPatchToolType::Freeform),
            supports_reasoning_summaries: true,
            ...
            supports_parallel_tool_calls: true,
            context_window: Some(CONTEXT_WINDOW_272K),
            supported_reasoning_levels: supported_reasoning_level_low_medium_high_non_hypercode(),
        )
        ...
        warn!("Unknown model {slug} is used. This will degrade the performance of Hypercode.");
        model_info!(
            slug,
            context_window: None,
...
fn supported_reasoning_level_low_medium_high_non_hypercode() -> Vec<ReasoningEffortPreset> {
    vec![
        ReasoningEffortPreset {
            effort: ReasoningEffort::Low,
...
fn supported_reasoning_level_low_medium_high_xhigh_non_hypercode() -> Vec<ReasoningEffortPreset> {
    vec![
        ReasoningEffortPreset {
            effort: ReasoningEffort::Low,
            description: "Balances speed with some reasoning; useful for straightforward queries and short explanations".to_string(),
