...
use hypercode_protocol::protocol::ApplyPatchApprovalRequestEvent;
use hypercode_protocol::protocol::Event;
use hypercode_protocol::protocol::EventMsg;
use hypercode_protocol::protocol::ExecApprovalRequestEvent;
...
use hypercode_protocol::protocol::Submission;
...
use crate::hypercode::SUBMISSION_CHANNEL_CAPACITY;
...
/// Start an interactive sub-Hypercode thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
pub(crate) async fn run_hypercode_thread_interactive(
    config: Config,
...
) -> Result<Hypercode, HypercodeErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);

    let HypercodeSpawnOk { hypercode, .. } = Hypercode::spawn(
        config,
        auth_manager,
        models_manager,
    ...
    .await?;
    let hypercode = Arc::new(hypercode);
    ...
    let hypercode_for_events = Arc::clone(&hypercode);
    tokio::spawn(async move {
        forward_events(
            hypercode_for_events,
            tx_sub,
    ...
    let hypercode_for_ops = Arc::clone(&hypercode);
    tokio::spawn(async move {
        forward_ops(hypercode_for_ops, rx_ops, cancel_token_ops).await;
    });
    ...
    Ok(Hypercode {
        ...
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: hypercode.agent_status.clone(),
    })
}
...
pub(crate) async fn run_hypercode_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_hypercode_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&hypercode).await;
                break;
            }
            event = hypercode.next_event() => {
...
async fn shutdown_delegate(hypercode: &Hypercode) {
    let _ = hypercode.submit(Op::Interrupt).await;
    let _ = hypercode.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = hypercode.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = hypercode.submit(op).await;
    }
}
...
async fn handle_exec_approval(
    ...
    let _ = hypercode.submit(Op::ExecApproval { id, decision }).await;
}
...
async fn handle_patch_approval(
    ...
    .await;
    let _ = hypercode.submit(Op::PatchApproval { id, decision }).await;
}
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> hypercode_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = hypercode_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, hypercode_protocol::protocol::ReviewDecision::Abort)
                .await;
            hypercode_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
        ...
        let (session, ctx, _rx_evt) = crate::hypercode::make_session_and_context_with_rx().await;
