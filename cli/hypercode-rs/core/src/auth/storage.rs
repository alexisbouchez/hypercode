...
use hypercode_keyring_store::DefaultKeyringStore;
use hypercode_keyring_store::KeyringStore;

/// Determine where Hypercode should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in HYPERCODE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in HYPERCODE_HOME.
    Auto,
}

/// Expected structure for $HYPERCODE_HOME/auth.json.
#[derive(Deserialize, Serialize, Clone, Debug, PartialEq)]
pub struct AuthDotJson {
    #[serde(rename = "OPENAI_API_KEY")]
    pub openai_api_key: Option<String>,

    #[serde(rename = "MISTRAL_API_KEY", default, skip_serializing_if = "Option::is_none")]
...
pub(super) fn get_auth_file(hypercode_home: &Path) -> PathBuf {
    hypercode_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(hypercode_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(hypercode_home: PathBuf) -> Self {
        Self { hypercode_home }
    }

    /// Attempt to read and refresh the `auth.json` file in the given `HYPERCODE_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.hypercode_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.hypercode_home)
    }
}

const KEYRING_SERVICE: &str = "Hypercode Auth";

// turns hypercode_home path into a stable, short key string
fn compute_store_key(hypercode_home: &Path) -> std::io::Result<String> {
    let canonical = hypercode_home
        .canonicalize()
        .unwrap_or_else(|_| hypercode_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(hypercode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            hypercode_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.hypercode_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.hypercode_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.hypercode_home)?;
...
impl AutoAuthStorage {
    fn new(hypercode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(hypercode_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(hypercode_home)),
        }
    }
}
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(hypercode_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(hypercode_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(hypercode_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(hypercode_home, keyring_store)),
    }
}
...
mod tests {
    ...
    use hypercode_keyring_store::tests::MockKeyringStore;
    use keyring::Error as KeyringError;

    #[tokio::test]
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        let hypercode_home = tempdir()?;
        let storage = FileAuthStorage::new(hypercode_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(hypercode_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(hypercode_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let hypercode_home = PathBuf::from("~/.hypercode");

        let key = compute_store_key(hypercode_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(hypercode_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(hypercode_home.path())?;
    ...
    fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
        let hypercode_home = tempdir()?;
        let mock_keyring = MockKeyringStore::default();
        let storage = AutoAuthStorage::new(hypercode_home.path().to_path_buf(), Arc::new(mock_keyring));
