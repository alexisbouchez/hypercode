...
async fn run_stream_with_bytes(sse_body: &[u8]) -> Vec<ResponseEvent> {
    ...
    let hypercode_home = match TempDir::new() {
        Ok(dir) => dir,
...
async fn chat_sse_emits_failed_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("hypercode.api_request") && line.contains("http.response.status_code=200")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("cannot find hypercode.api_request event".to_string()))
    });
...
async fn chat_sse_done_chunk_emits_event() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("hypercode.sse_event") && line.contains("event.kind=message"))
            .map(|_| Ok(()))
            .unwrap_or(Err("cannot find SSE event".to_string()))
    });
}
