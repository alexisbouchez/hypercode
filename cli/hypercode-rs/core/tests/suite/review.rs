...
use hypercode_core::REVIEW_PROMPT;
...
use hypercode_core::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use hypercode_core::protocol::EventMsg;
use hypercode_core::protocol::ExitedReviewModeEvent;
use hypercode_core::protocol::Op;
use hypercode_core::protocol::ReviewCodeLocation;
use hypercode_core::protocol::ReviewFinding;
use hypercode_core::protocol::ReviewLineRange;
use hypercode_core::protocol::ReviewOutputEvent;
...
use hypercode_core::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Hypercode sandbox network restrictions.
    skip_if_no_network!();
    ...
    let hypercode = new_conversation_for_server(&server, &hypercode_home, |_| {}).await;
    ...
    let _entered = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = hypercode.rollout_path();
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&hypercode, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let hypercode = new_conversation_for_server(&server, &hypercode_home, |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&hypercode, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = hypercode_home.path().join("resume.jsonl");
    {
        ...
        let user = hypercode_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = hypercode_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let hypercode =
        resume_conversation_for_server(&server, &hypercode_home, session_file.clone(), |_| {}).await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let hypercode_home = TempDir::new().unwrap();
    let hypercode = new_conversation_for_server(&server, &hypercode_home, |config| {
