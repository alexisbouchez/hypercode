...
use hypercode_core::LocalShellAction;
use hypercode_core::LocalShellExecAction;
use hypercode_core::LocalShellStatus;
...
use hypercode_core::built_in_model_providers;
...
use hypercode_core::protocol::SessionSource;
...
use hypercode_protocol::models::ReasoningItemContent;
use hypercode_protocol::models::ReasoningItemReasoningSummary;
use hypercode_protocol::models::WebSearchAction;
...
use core_test_support::test_hypercode::TestHypercode;
use core_test_support::test_hypercode::test_hypercode;
...
/// Writes an `auth.json` into the provided `hypercode_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        hypercode_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = hypercode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![hypercode_protocol::models::ContentItem::InputText {
    ...
    let prior_system = hypercode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![hypercode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = hypercode_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Hypercode to resume from our file
    ...
    let auth_manager =
        hypercode_core::AuthManager::from_auth_for_testing(HypercodeAuth::from_api_key("Test API Key"));
    let NewThread {
        thread: hypercode,
        session_configured,
        ..
    ...
    hypercode
        ...
        .await
        .unwrap();
    wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator, "hypercode_cli_rs");
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/api/hypercode", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/hypercode/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match HypercodeAuth::from_auth_storage(hypercode_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => hypercode_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No HypercodeAuth found in hypercode_home"),
            Err(e) => panic!("Failed to load HypercodeAuth: {e}"),
        };
    ...
    let NewThread { thread: hypercode, .. } = thread_manager
        .start_thread(config)
        .await
        .expect("create new conversation");
...
async fn skills_append_to_instructions() {
    ...
    let hypercode_home = TempDir::new().unwrap();
    let skill_dir = hypercode_home.path().join("skills/demo");
    ...
    let mut config = load_default_config_for_test(&hypercode_home).await;
    config.model_provider = model_provider;
    config.cwd = hypercode_home.path().to_path_buf();
    ...
    let _hypercode_home_guard = hypercode_home;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestHypercode { hypercode, .. } = test_hypercode()
...
-> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestHypercode { hypercode, .. } = test_hypercode().with_model("gpt-5.1").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(HypercodeAuth::from_api_key("Test API Key"));
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-hypercode-primary-used-percent", "12.5")
        .insert_header("x-hypercode-secondary-used-percent", "40.0")
        .insert_header("x-hypercode-primary-window-minutes", "10")
        .insert_header("x-hypercode-secondary-window-minutes", "60")
        .insert_header("x-hypercode-primary-reset-at", "1704069000")
        .insert_header("x-hypercode-secondary-reset-at", "1704074400")
    ...
    let first_token_event =
        wait_for_event(&hypercode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    wait_for_event(&hypercode, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-hypercode-primary-used-percent", "100.0")
        .insert_header("x-hypercode-secondary-used-percent", "87.5")
        .insert_header("x-hypercode-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-hypercode-primary-window-minutes", "15")
        .insert_header("x-hypercode-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_hypercode();
    let hypercode_fixture = builder.build(&server).await?;
    let hypercode = hypercode_fixture.hypercode.clone();
    ...
    let token_event = wait_for_event(&hypercode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&hypercode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&hypercode, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = HypercodeErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_hypercode_auth() -> HypercodeAuth {
    HypercodeAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Hypercode sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
    ...
    // Init session with isolated hypercode home.
    let hypercode_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&hypercode_home).await;
    config.model_provider = model_provider;
