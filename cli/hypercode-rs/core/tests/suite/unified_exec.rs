...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    let builder = test_hypercode().with_config(|config| {
        config.include_apply_patch_tool = true;
        config.use_experimental_unified_exec_tool = true;
        config.features.enable(Feature::UnifiedExec);
    });
    let harness = TestHypercodeHarness::with_builder(builder).await?;
    ...
    let hypercode = test.hypercode.clone();
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_model("gpt-5").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config.features.enable(Feature::UnifiedExec);
    });
    let TestHypercode {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config.features.enable(Feature::UnifiedExec);
    });
    let TestHypercode {
    ...
    let end_event = wait_for_event_match(&hypercode, |msg| match msg {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&hypercode, |_| true).await;
        match msg {
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    loop {
        let event_msg = wait_for_event(&hypercode, |_| true).await;
        match event_msg {
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = hypercode.next_event().await.expect("event");
