...
fn write_minimal_rollout_with_id(hypercode_home: &Path, id: Uuid) -> PathBuf {
    let sessions = hypercode_home.join("sessions/2024/01/01");
...
#[tokio::test]
async fn find_handles_gitignore_covering_hypercode_home_directory() {
    let repo = TempDir::new().unwrap();
    let hypercode_home = repo.path().join(".hypercode");
    std::fs::create_dir_all(&hypercode_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".hypercode/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&hypercode_home, id);

    let found = find_thread_path_by_id_str(&hypercode_home, &id.to_string())
        .await
        .unwrap();
