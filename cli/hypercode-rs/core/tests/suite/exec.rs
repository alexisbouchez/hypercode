...
use hypercode_core::exec::ExecToolCallOutput;
use hypercode_core::exec::SandboxType;
...
use hypercode_core::spawn::HYPERCODE_SANDBOX_ENV_VAR;
...
fn skip_test() -> bool {
    if std::env::var(HYPERCODE_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{HYPERCODE_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
