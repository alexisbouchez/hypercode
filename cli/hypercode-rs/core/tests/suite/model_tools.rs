...
async fn collect_tool_identifiers_for_model(model: &str) -> Vec<String> {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Hypercode conversation");
...
async fn model_selects_expected_tools() {
    ...
    let hypercode_tools = collect_tool_identifiers_for_model("hypercode-mini-latest").await;
    assert_eq!(
        hypercode_tools,
        vec![
        ...
        "hypercode-mini-latest should expose the local shell tool",
    );

    let gpt5_hypercode_tools = collect_tool_identifiers_for_model("gpt-5-hypercode").await;
    assert_eq!(
        gpt5_hypercode_tools,
        vec![
        ...
        "gpt-5-hypercode should expose the apply_patch tool",
    );

    let gpt51_hypercode_tools = collect_tool_identifiers_for_model("gpt-5.1-codex").await;
    assert_eq!(
        gpt51_hypercode_tools,
        vec![
            "shell_command".to_string(),
            "list_mcp_resources".to_string(),
            "list_mcp_resource_templates".to_string(),
