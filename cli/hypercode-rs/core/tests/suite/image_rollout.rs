...
async fn copy_paste_local_image_persists_rollout_request_shape() -> anyhow::Result<()> {
    ...
    wait_for_event(&hypercode, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = hypercode.rollout_path();
    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: hypercode_protocol::models::local_image_open_tag_text(1),
            },
...
async fn drag_drop_image_persists_rollout_request_shape() -> anyhow::Result<()> {
    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: hypercode_protocol::models::image_open_tag_text(),
            },
            ContentItem::InputImage { image_url },
            ContentItem::InputText {
                text: hypercode_protocol::models::image_close_tag_text(),
