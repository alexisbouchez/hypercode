...
async fn hypercode_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let mut builder = test_hypercode().with_model("gpt-5.1").with_config(|config| {
        config.approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
        config.sandbox_policy = Constrained::allow_any(SandboxPolicy::ReadOnly);
    });
    let test = builder.build(&server).await.expect("build test hypercode");
    ...
    wait_for_event(&test.hypercode, |ev| {
    ...
    .await;
    wait_for_event(&test.hypercode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
}

/// Delegate should surface ApplyPatchApprovalRequest and honor parent decision
/// so the sub-agent can proceed to completion.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hypercode_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hypercode_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.hypercode, |_| true).await;
        match ev {
