...
use hypercode_core::compact::SUMMARIZATION_PROMPT;
use hypercode_core::compact::SUMMARY_PREFIX;
...
use hypercode_core::protocol::RolloutItem;
use hypercode_core::protocol::RolloutLine;
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Hypercode.
    ...
    let thread_manager = ThreadManager::with_models_provider(
        HypercodeAuth::from_api_key("dummy"),
    ...
    hypercode.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Hypercode to flush rollout entries before inspecting the file.
    hypercode.submit(Op::Shutdown).await.unwrap();
...
async fn manual_compact_uses_custom_prompt() {
    ...
    hypercode.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let NewThread { thread: hypercode, .. } = thread_manager.start_thread(config).await.unwrap();
    ...
    let first = wait_for_event_match(&hypercode, |ev| match ev {
    ...
    let last = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_token_limit_hit() {
    ...
    let hypercode = thread_manager.start_thread(config).await.unwrap().thread;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        hypercode_protocol::models::ResponseItem::Message {
            id: None,
        ...
        hypercode_protocol::models::ResponseItem::Compaction {
    ...
    let mut builder = test_hypercode().with_config(move |config| {
    ...
    let mut resume_builder = test_hypercode().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        config.features.enable(Feature::RemoteCompaction);
    });
    ...
    wait_for_event(&resumed.hypercode, |event| {
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    set_test_compact_prompt(&mut config);
    config.model_auto_compact_token_limit = Some(200_000);
    let hypercode = ThreadManager::with_models_provider(
        HypercodeAuth::from_api_key("dummy"),
        config.model_provider.clone(),
    )
    .start_thread(config)
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn auto_compact_allows_multiple_attempts_when_interleaved_with_other_turn_events() {
    ...
    hypercode
        ...
        loop {
            let event = hypercode.next_event().await.unwrap();
...
async fn auto_compact_counts_encrypted_reasoning_before_last_user() {
    ...
    let hypercode = test_hypercode()
        .with_auth(HypercodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            set_test_compact_prompt(config);
