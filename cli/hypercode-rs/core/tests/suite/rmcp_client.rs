...
use hypercode_utils_cargo_bin::cargo_bin;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.hypercode, |ev| {
    ...
    let end_event = wait_for_event(&fixture.hypercode, |ev| {
    ...
    wait_for_event(&fixture.hypercode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_image_completions_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_hypercode()
        .with_config(move |config| {
            config.model_provider.wire_api = hypercode_core::WireApi::Chat;
            let mut servers = config.mcp_servers.get().clone();
            servers.insert(
                server_name.to_string(),
                McpServerConfig {
...
/// This test writes to a fallback credentials file in HYPERCODE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire HYPERCODE_HOME through the code.
#[serial(hypercode_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("HYPERCODE_HOME", temp_home.path().as_os_str());
