...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestHypercode) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestHypercode, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.hypercode, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestHypercode) {
    let event = wait_for_event(&test.hypercode, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestHypercode) {
    wait_for_event(&test.hypercode, |event| {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_hypercode().with_model(model).with_config(move |config| {
