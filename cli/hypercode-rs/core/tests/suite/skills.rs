...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_pre_build_hook(|home| {
        write_skill(home, "demo", "demo skill", skill_body);
    });
    let test = builder.build(&server).await?;

    let skill_path = test.hypercode_home_path().join("skills/demo/SKILL.md");
    ...
    test.hypercode
        .submit(Op::UserTurn {
            ...
            model: session_model,
            effort: None,
            summary: hypercode_protocol::config_types::ReasoningSummary::Auto,
        })
        .await?;

    core_test_support::wait_for_event(test.hypercode.as_ref(), |event| {
        matches!(event, hypercode_core::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    test.hypercode
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.hypercode.as_ref(), |event| match event {
            hypercode_core::protocol::EventMsg::ListSkillsResponse(response) => Some(response.clone()),
            _ => None,
        })
        .await;
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.hypercode_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, hypercode_protocol::protocol::SkillScope::System);
