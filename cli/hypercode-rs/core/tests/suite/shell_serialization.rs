...
use core_test_support::test_hypercode::ApplyPatchModelOutput;
use core_test_support::test_hypercode::ShellModelOutput;
use core_test_support::test_hypercode::TestHypercodeBuilder;
...
fn configure_shell_model(
    builder: TestHypercodeBuilder,
    output_type: ShellModelOutput,
    include_apply_patch_tool: bool,
) -> TestHypercodeBuilder {
    let builder = match (output_type, include_apply_patch_tool) {
        (ShellModelOutput::ShellCommand, _) => builder.with_model("test-gpt-5-hypercode"),
        ...
        (ShellModelOutput::LocalShell, false) => builder.with_model("hypercode-mini-latest"),
...
async fn shell_output_stays_json_without_freeform_apply_patch(
    ...
    let mut builder = configure_shell_model(test_hypercode(), output_type, false);
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let server = start_mock_server().await;
    let mut builder = configure_shell_model(test_hypercode(), output_type, true);
    let test = builder.build(&server).await?;
...
async fn shell_output_reserializes_truncated_content(output_type: ShellModelOutput) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder =
        configure_shell_model(test_hypercode(), output_type, true).with_config(move |config| {
...
async fn shell_command_output_is_not_truncated_under_10k_bytes() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_hypercode().with_model("gpt-5.1");
    let test = builder.build(&server).await?;
