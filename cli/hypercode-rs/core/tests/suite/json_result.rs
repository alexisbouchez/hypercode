...
async fn hypercode_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    hypercode_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hypercode_returns_json_result_for_gpt5_hypercode() -> anyhow::Result<()> {
    hypercode_returns_json_result("gpt-5.1-codex".to_string()).await
}

async fn hypercode_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("hypercode_output_schema".into()))
    ...
    let TestHypercode { hypercode, cwd, .. } = test_hypercode().build(&server).await?;
    ...
    let message = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
