...
async fn request_body_is_zstd_compressed_for_hypercode_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/hypercode/v1", server.uri());
    ...
    let hypercode = builder.build(&server).await?.hypercode;
