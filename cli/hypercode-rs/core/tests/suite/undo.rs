...
use core_test_support::test_hypercode::TestHypercodeHarness;
use core_test_support::test_hypercode::test_hypercode;
use core_test_support::wait_for_event_match;
use pretty_assertions::assert_eq;

#[allow(clippy::expect_used)]
async fn undo_harness() -> Result<TestHypercodeHarness> {
    let builder = test_hypercode().with_model("gpt-5.1").with_config(|config| {
        config.include_apply_patch_tool = true;
        config.features.enable(Feature::GhostCommit);
    });
    TestHypercodeHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Hypercode Tests"])?;
    git(path, &["config", "user.email", "hypercode-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Hypercode.\n")?;
...
async fn invoke_undo(hypercode: &Arc<HypercodeThread>) -> Result<UndoCompletedEvent> {
    hypercode.submit(Op::Undo).await?;
    let event = wait_for_event_match(hypercode, |msg| match msg {
...
async fn expect_successful_undo(hypercode: &Arc<HypercodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(hypercode).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(hypercode: &Arc<HypercodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(hypercode).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let hypercode = Arc::clone(&harness.test().hypercode);
    let completed = expect_successful_undo(&hypercode).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let hypercode = Arc::clone(&harness.test().hypercode);
    expect_successful_undo(&hypercode).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&hypercode).await?;
