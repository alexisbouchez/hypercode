...
async fn run_turn(test: &TestHypercode, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestHypercode, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_hypercode_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestHypercode> {
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_hypercode_with_test_tool(&server).await?;
