...
async fn responses_api_emits_api_request_event() {
    ...
    let TestHypercode { hypercode, .. } = test_hypercode().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("hypercode.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected hypercode.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("hypercode.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected hypercode.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("hypercode.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing hypercode.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("hypercode.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing hypercode.sse_event".to_string()))
    });
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("hypercode.tool_result") && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing hypercode.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("hypercode.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing hypercode.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_missing_ids() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("hypercode.tool_result")
...
async fn handle_response_item_records_tool_result_for_local_shell_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("hypercode.tool_result") && line.contains("call_id=shell-call"))
            .ok_or_else(|| "missing hypercode.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("hypercode.tool_decision") && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing hypercode.tool_decision event for {call_id}"))?;
...
async fn handle_container_exec_user_approved_records_tool_decision() {
    ...
    wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::ExecApprovalRequest(_))).await;
