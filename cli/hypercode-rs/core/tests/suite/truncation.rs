...
async fn truncate_function_error_trims_respond_to_model() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_model("test-gpt-5.1-codex");
...
async fn tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    // Use a model that exposes the shell_command tool.
    let mut builder = test_hypercode().with_model("gpt-5.1-codex");
    let fixture = builder.build(&server).await?;
...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_config(move |config| {
        ...
        servers.insert(
            ...
            hypercode_core::config::types::McpServerConfig {
                transport: hypercode_core::config::types::McpServerTransportConfig::Stdio {
