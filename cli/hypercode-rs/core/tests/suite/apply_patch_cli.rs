...
pub async fn apply_patch_harness() -> Result<TestHypercodeHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestHypercodeBuilder) -> TestHypercodeBuilder,
) -> Result<TestHypercodeHarness> {
    let builder = configure(test_hypercode()).with_config(|config| {
