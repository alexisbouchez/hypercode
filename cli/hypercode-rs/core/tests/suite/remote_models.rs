...
use hypercode_protocol::openai_models::ConfigShellToolType;
use hypercode_protocol::openai_models::ModelInfo;
use hypercode_protocol::openai_models::ModelPreset;
use hypercode_protocol::openai_models::ModelVisibility;
use hypercode_protocol::openai_models::ModelsResponse;
use hypercode_protocol::openai_models::ReasoningEffort;
use hypercode_protocol::openai_models::ReasoningEffortPreset;
use hypercode_protocol::openai_models::TruncationPolicyConfig;
...
const REMOTE_MODEL_SLUG: &str = "hypercode-test";
...
async fn remote_models_remote_model_uses_unified_exec() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&hypercode, |msg| match msg {
    ...
    wait_for_event(&hypercode, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "hypercode-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "hypercode-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        1,
...
async fn remote_models_preserve_builtin_presets() -> Result<()> {
    ...
    let manager = ModelsManager::with_provider(
        ...
        hypercode_core::auth::AuthManager::from_auth_for_testing(auth),
        provider,
    );
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let remote_model = test_remote_model("hypercode-auto-balanced", ModelVisibility::Hide, 0);
    mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "hypercode-auto-balanced")
