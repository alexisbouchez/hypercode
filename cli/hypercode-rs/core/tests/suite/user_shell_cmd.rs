...
use hypercode_core::protocol::ExecOutputStream;
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    let thread_manager = ThreadManager::with_models_provider(
        hypercode_core::HypercodeAuth::from_api_key("dummy"),
    ...
    hypercode
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _ = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;
    hypercode.submit(Op::Interrupt).await.unwrap();

    // Expect a TurnAborted(Interrupted) notification.
    let msg = wait_for_event(&hypercode, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_hypercode::test_hypercode();
    let test = builder.build(&server).await?;

    #[cfg(windows)]
    let command = r#"$val = $env:HYPERCODE_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${HYPERCODE_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.hypercode, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.hypercode, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.hypercode, |ev| match ev {
    ...
    let _ = wait_for_event(&test.hypercode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_hypercode::test_hypercode();
    let test = builder
        .with_config(|config| {
