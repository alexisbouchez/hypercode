...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_hypercode(&server).await?;
...
async fn build_test_hypercode(server: &wiremock::MockServer) -> Result<TestHypercode> {
    let mut builder = test_hypercode().with_model(MODEL_WITH_TOOL);
