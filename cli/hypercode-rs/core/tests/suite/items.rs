...
use hypercode_core::protocol::ItemCompletedEvent;
use hypercode_core::protocol::ItemStartedEvent;
...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started_item = wait_for_event_match(&hypercode, |ev| match ev {
    ...
    .await;
    let completed_item = wait_for_event_match(&hypercode, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
    ...
    .await;
    let completed = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::AgentMessage(item),
            ..
        }) => Some(item.clone()),
    ...
    let Some(hypercode_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let TestHypercode {
        ...
        session_configured,
        ..
    } = test_hypercode().build(&server).await?;
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::AgentMessageContentDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;
    let legacy_delta = wait_for_event_match(&hypercode, |ev| match ev {
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),
