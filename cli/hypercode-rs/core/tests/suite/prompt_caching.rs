...
use hypercode_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
use hypercode_core::features::Feature;
use hypercode_core::models_manager::model_info::BASE_INSTRUCTIONS;
...
use hypercode_core::shell::Shell;
use hypercode_core::shell::default_user_shell;
...
async fn hypercode_mini_latest_tools() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    use pretty_assertions::assert_eq;
    ...
    let TestHypercode { hypercode, .. } = test_hypercode()
        .with_config(|config| {
            ...
            config.model = Some("hypercode-mini-latest".to_string());
        })
        .build(&server)
        .await?;
...
-> anyhow::Result<()> {
    ...
    let TestHypercode { hypercode, config, .. } = test_hypercode()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
        })
        .build(&server)
...
async fn override_before_first_turn_emits_environment_context() -> anyhow::Result<()> {
    ...
    let TestHypercode { hypercode, .. } = test_hypercode().build(&server).await?;
