...
//! request payload that Hypercode would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (TempDir, Config, ThreadManager, Arc<HypercodeThread>) {
...
async fn user_turn(conversation: &Arc<HypercodeThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<HypercodeThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<HypercodeThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager =
        hypercode_core::AuthManager::from_auth_for_testing(HypercodeAuth::from_api_key("dummy"));
    let NewThread { thread, .. } = manager
