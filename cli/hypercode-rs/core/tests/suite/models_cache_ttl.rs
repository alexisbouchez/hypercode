...
const REMOTE_MODEL: &str = "hypercode-test-ttl";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let mut builder = test_hypercode().with_auth(HypercodeAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let hypercode = Arc::clone(&test.hypercode);
    ...
    let cache_path = config.hypercode_home.join(CACHE_FILE);
    ...
    hypercode
        .submit(Op::UserTurn {
            ...
            approval_policy: hypercode_core::protocol::AskForApproval::Never,
    ...
    let _ = wait_for_event(&hypercode, |event| matches!(event, EventMsg::TurnComplete(_))).await;
