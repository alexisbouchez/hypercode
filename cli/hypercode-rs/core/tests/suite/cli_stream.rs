...
use hypercode_core::protocol::GitInfo;
...
/// 2. Configures hypercode to use this mock server via a custom provider
...
async fn chat_mode_stream_cli() {
    ...
    let bin = hypercode_utils_cargo_bin::cargo_bin("hypercode").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("HYPERCODE_HOME", home.path())
...
async fn exec_cli_applies_experimental_instructions_file() {
    ...
    // Hypercode to use the Responses API with the dummy env var.
    let provider_override = format!(
...
/// 2. Configures hypercode to read from this fixture via CODEX_RS_SSE_FIXTURE env var
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the hypercode CLI and invoke `exec`, which is what records a session.
    let bin = hypercode_utils_cargo_bin::cargo_bin("hypercode").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = hypercode_utils_cargo_bin::cargo_bin("hypercode").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("HYPERCODE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("CODEX_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume hypercode-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = hypercode_core::git_info::collect_git_info(&git_repo).await;
