...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    ...
    let hypercode_home = test.home.path().to_path_buf();
    ...
    let begin = wait_for_event_match(&hypercode, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    let mut entries = fs::read_dir(hypercode_home.join("shell_snapshots")).await?;
    ...
    let end = wait_for_event_match(&hypercode, |ev| match ev {
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.hypercode_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let hypercode_home = home.path().to_path_buf();
    let hypercode = harness.test().hypercode.clone();
    ...
    hypercode.submit(Op::Shutdown {}).await?;
