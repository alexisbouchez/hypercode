...
use hypercode_utils_cargo_bin::CargoBinError;
use hypercode_utils_cargo_bin::find_resource;
...
pub mod test_hypercode;
pub mod test_hypercode_exec;
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\hypercode\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.hypercode`.
pub async fn load_default_config_for_test(hypercode_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        hypercode_linux_sandbox_exe: Some(
            hypercode_utils_cargo_bin::cargo_bin("hypercode-linux-sandbox")
                .expect("should find binary for hypercode-linux-sandbox"),
        ),
...
pub async fn wait_for_event<F>(hypercode: &HypercodeThread, predicate: F) -> hypercode_core::protocol::EventMsg
where
    F: FnMut(&hypercode_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(hypercode, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(hypercode: &HypercodeThread, matcher: F) -> T
where
    F: Fn(&hypercode_core::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(hypercode, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}

pub async fn wait_for_event_with_timeout<F>(
    hypercode: &HypercodeThread,
    mut predicate: F,
    wait_time: tokio::time::Duration,
) -> hypercode_core::protocol::EventMsg
where
    F: FnMut(&hypercode_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(5)), hypercode.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    hypercode_core::spawn::HYPERCODE_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    hypercode_core::spawn::HYPERCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    hypercode_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    hypercode_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    hypercode_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
