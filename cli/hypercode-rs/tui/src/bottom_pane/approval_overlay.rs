...
use hypercode_core::protocol::ElicitationAction;
...
impl ApprovalOverlay {
    ...
    fn handle_exec_decision(&self, id: &str, command: &[String], decision: ReviewDecision) {
        ...
        self.app_event_tx.send(AppEvent::HypercodeOp(Op::ExecApproval {
            id: id.to_string(),
            decision,
        }));
    }
    ...
    fn handle_patch_decision(&self, id: &str, decision: ReviewDecision) {
        self.app_event_tx.send(AppEvent::HypercodeOp(Op::PatchApproval {
            id: id.to_string(),
            decision,
        }));
    }
    ...
    fn handle_elicitation_decision(
        ...
        self.app_event_tx
            .send(AppEvent::HypercodeOp(Op::ResolveElicitation {
...
fn exec_options(
    ...
    .chain([ApprovalOption {
        label: "No, and tell Hypercode what to do differently".to_string(),
...
mod tests {
    ...
    fn shortcut_triggers_selection() {
        ...
        // We expect at least one HypercodeOp message in the queue.
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if matches!(ev, AppEvent::HypercodeOp(_)) {
                saw_op = true;
                break;
            }
        }
    ...
    fn exec_prefix_option_emits_execpolicy_amendment() {
        ...
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::HypercodeOp(Op::ExecApproval { decision, .. }) = ev {
                assert_eq!(
                    decision,
                    ReviewDecision::ApprovedExecpolicyAmendment {
    ...
    fn exec_history_cell_wraps_with_two_space_indent() {
        ...
        let rendered: Vec<String> = lines
            ...
            .collect();
        let expected = vec![
            "✔ You approved hypercode to run".to_string(),
    ...
    fn enter_sets_last_selected_index_without_dismissing() {
        ...
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::HypercodeOp(Op::ExecApproval { decision: d, .. }) = ev {
                decision = Some(d);
                break;
            }
        }
