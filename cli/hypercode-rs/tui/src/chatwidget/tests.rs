...
//!
//! These tests treat the widget as the adapter between `hypercode_core::protocol::EventMsg` inputs and
...
use hypercode_core::config::Constrained;
use hypercode_core::config::ConstraintError;
use hypercode_core::config_loader::RequirementSource;
...
use hypercode_core::protocol::ExecPolicyAmendment;
...
use hypercode_core::protocol::PatchApplyEndEvent;
...
use hypercode_core::protocol::TurnStartedEvent;
...
use hypercode_protocol::openai_models::ReasoningEffortPreset;
...
fn set_windows_sandbox_enabled(enabled: bool) {
    hypercode_core::set_windows_sandbox_enabled(enabled);
}

#[cfg(target_os = "windows")]
fn set_windows_elevated_sandbox_enabled(enabled: bool) {
    hypercode_core::set_windows_elevated_sandbox_enabled(enabled);
}

async fn test_config() -> Config {
    // Use base defaults to avoid depending on host state.
    let hypercode_home = std::env::temp_dir();
    ConfigBuilder::default()
        .hypercode_home(hypercode_home.clone())
        .build()
        .await
        .expect("config")
}
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = hypercode_core::protocol::SessionConfiguredEvent {
    ...
    chat.handle_hypercode_event(Event {
        id: "initial".into(),
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let thread_manager = Arc::new(ThreadManager::with_models_provider(
        HypercodeAuth::from_api_key("test"),
        cfg.model_provider.clone(),
    ));
    let auth_manager = AuthManager::from_auth_for_testing(HypercodeAuth::from_api_key("test"));
    let init = ChatWidgetInit {
        ...
        feedback: hypercode_feedback::HypercodeFeedback::new(),
        is_first_run: true,
...
async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask Hypercode to do anything".to_string(),
    ...
    let auth_manager = AuthManager::from_auth_for_testing(HypercodeAuth::from_api_key("test"));
    let hypercode_home = cfg.hypercode_home.clone();
    let widget = ChatWidget {
        app_event_tx,
        hypercode_op_tx: op_tx,
        bottom_pane: bottom,
        active_cell: None,
        ...
        models_manager: Arc::new(ModelsManager::new(hypercode_home, auth_manager)),
...
fn set_chatgpt_auth(chat: &mut ChatWidget) {
    chat.auth_manager =
        AuthManager::from_auth_for_testing(HypercodeAuth::create_dummy_chatgpt_auth_for_testing());
    chat.models_manager = Arc::new(ModelsManager::new(
        chat.config.hypercode_home.clone(),
...
async fn rate_limit_switch_prompt_shows_once_per_session() {
    let auth = HypercodeAuth::create_dummy_chatgpt_auth_for_testing();
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = hypercode_core::parse_command::parse_command(&command);
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = hypercode_core::parse_command::parse_command(&command);
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    chat.handle_hypercode_event(Event {
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Hypercode op to be sent, got {other:?}"),
    }
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/hypercode-test-rollout.jsonl");
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        hypercode_core::git_info::CommitLogEntry {
...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::HypercodeOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::HypercodeOp(Op::Review { review_request }) => {
            assert_eq!(
                review_request,
                ReviewRequest {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::HypercodeOp should be sent
...
async fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_hypercode_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(hypercode_core::protocol::TurnAbortedEvent {
...
async fn model_selection_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5-hypercode")).await;
    chat.thread_id = Some(ThreadId::new());
...
async fn approvals_selection_popup_snapshot_windows_degraded_sandbox() {
    ...
    let was_sandbox_enabled = hypercode_core::get_platform_sandbox().is_some();
    let was_elevated_enabled = hypercode_core::is_windows_elevated_sandbox_enabled();
...
async fn approvals_popup_navigation_skips_disabled() {
    ...
    assert!(
        app_events.iter().any(|ev| matches!(
            ev,
            AppEvent::HypercodeOp(Op::OverrideTurnContext {
...
//
// Synthesizes a Hypercode ExecApprovalRequest event to trigger the approval modal
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a HypercodeOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::HypercodeOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert_matches!(decision, hypercode_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a HypercodeOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::HypercodeOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected HypercodeOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto hypercode_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to hypercode channel");
    match forwarded {
