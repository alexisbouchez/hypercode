...
use hypercode_core::config::edit::ConfigEdit;
...
fn session_summary(token_usage: TokenUsage, thread_id: Option<ThreadId>) -> Option<SessionSummary> {
    ...
    let resume_command = thread_id.map(|thread_id| format!("hypercode resume {thread_id}"));
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        ...
        match event {
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                let mut builder = ConfigEditsBuilder::new(&self.config.hypercode_home)
...
mod tests {
    ...
    async fn model_migration_prompt_skips_when_target_missing() {
        ...
        let mut current = available
            .iter()
            .find(|preset| preset.model == "gpt-5-hypercode")
            .cloned()
        ...
        available.retain(|preset| preset.model != "gpt-5-hypercode");
