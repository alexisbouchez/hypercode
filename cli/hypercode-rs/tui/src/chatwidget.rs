//! The main Hypercode TUI chat surface.
//!
...
use hypercode_core::config::ConstraintResult;
use hypercode_core::config::types::Notifications;
use hypercode_core::features::FEATURES;
use hypercode_core::features::Feature;
use hypercode_core::git_info::current_branch_name;
use hypercode_core::git_info::local_git_branches;
use hypercode_core::models_manager::manager::ModelsManager;
use hypercode_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use hypercode_core::protocol::AgentMessageDeltaEvent;
use hypercode_core::protocol::AgentMessageEvent;
use hypercode_core::protocol::AgentReasoningDeltaEvent;
use hypercode_core::protocol::AgentReasoningEvent;
use hypercode_core::protocol::AgentReasoningRawContentDeltaEvent;
use hypercode_core::protocol::AgentReasoningRawContentEvent;
use hypercode_core::protocol::ApplyPatchApprovalRequestEvent;
use hypercode_core::protocol::BackgroundEventEvent;
use hypercode_core::protocol::CreditsSnapshot;
use hypercode_core::protocol::DeprecationNoticeEvent;
use hypercode_core::protocol::ErrorEvent;
use hypercode_core::protocol::Event;
use hypercode_core::protocol::EventMsg;
use hypercode_core::protocol::ExecApprovalRequestEvent;
use hypercode_core::protocol::ExecCommandBeginEvent;
use hypercode_core::protocol::ExecCommandEndEvent;
use hypercode_core::protocol::ExecCommandOutputDeltaEvent;
use hypercode_core::protocol::ExecCommandSource;
use hypercode_core::protocol::ExitedReviewModeEvent;
use hypercode_core::protocol::ListCustomPromptsResponseEvent;
use hypercode_core::protocol::ListSkillsResponseEvent;
use hypercode_core::protocol::McpListToolsResponseEvent;
use hypercode_core::protocol::McpStartupCompleteEvent;
use hypercode_core::protocol::McpStartupStatus;
use hypercode_core::protocol::McpStartupUpdateEvent;
use hypercode_core::protocol::McpToolCallBeginEvent;
use hypercode_core::protocol::McpToolCallEndEvent;
use hypercode_core::protocol::Op;
use hypercode_core::protocol::PatchApplyBeginEvent;
use hypercode_core::protocol::RateLimitSnapshot;
use hypercode_core::protocol::ReviewRequest;
use hypercode_core::protocol::ReviewTarget;
use hypercode_core::protocol::SkillsListEntry;
use hypercode_core::protocol::StreamErrorEvent;
use hypercode_core::protocol::TerminalInteractionEvent;
use hypercode_core::protocol::TokenUsage;
use hypercode_core::protocol::TokenUsageInfo;
use hypercode_core::protocol::TurnAbortReason;
use hypercode_core::protocol::TurnCompleteEvent;
use hypercode_core::protocol::TurnDiffEvent;
use hypercode_core::protocol::UndoCompletedEvent;
use hypercode_core::protocol::UndoStartedEvent;
use hypercode_core::protocol::UserMessageEvent;
use hypercode_core::protocol::ViewImageToolCallEvent;
use hypercode_core::protocol::WarningEvent;
use hypercode_core::protocol::WebSearchBeginEvent;
use hypercode_core::protocol::WebSearchEndEvent;
use hypercode_core::skills::model::SkillMetadata;
use hypercode_protocol::ThreadId;
use hypercode_protocol::account::PlanType;
use hypercode_protocol::approvals::ElicitationRequestEvent;
use hypercode_protocol::parse_command::ParsedCommand;
use hypercode_protocol::user_input::UserInput;
...
use chrono::Local;
use hypercode_common::approval_presets::ApprovalPreset;
use hypercode_common::approval_presets::builtin_approval_presets;
...
use hypercode_core::protocol::SandboxPolicy;
use hypercode_file_search::FileMatch;
use hypercode_protocol::openai_models::ModelPreset;
use hypercode_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use hypercode_protocol::plan_tool::UpdatePlanArgs;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: hypercode_feedback::HypercodeFeedback,
...
/// and by sending requests back to hypercode-core.
///
...
pub(crate) struct ChatWidget {
    ...
    hypercode_op_tx: UnboundedSender<Op>,
    ...
    /// Tracks whether hypercode-core currently considers an agent turn to be in progress.
    ///
    ...
    feedback: hypercode_feedback::HypercodeFeedback,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: hypercode_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask hypercode-core to enumerate custom prompts for this session.
    ...
    fn on_patch_apply_end(&mut self, event: hypercode_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: hypercode_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let hypercode_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: hypercode_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let hypercode_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
    ...
    pub(crate) fn new_from_existing(
        ...
        conversation: std::sync::Arc<hypercode_core::HypercodeThread>,
        session_configured: hypercode_core::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::HypercodeOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    let windows_degraded_sandbox_enabled = hypercode_core::get_platform_sandbox()
                        .is_some()
                        && !hypercode_core::is_windows_elevated_sandbox_enabled();
                    if !windows_degraded_sandbox_enabled
                        || !hypercode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                    {
            ...
            SlashCommand::Logout => {
                if let Err(e) = hypercode_core::auth::logout(
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::HypercodeOp(Op::Undo));
            // }
            SlashCommand::Diff => {
            ...
            SlashCommand::TestApproval => {
                ...
                self.app_event_tx.send(AppEvent::HypercodeEvent(Event {
                    id: "1".to_string(),
    ...
    pub(crate) fn handle_hypercode_event(&mut self, event: Event) {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_hypercode_event: {:?}", msg);
            }
        }
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| hypercode_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::HypercodeOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("hypercode-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "hypercode-auto-fast" => 0,
            "hypercode-auto-balanced" => 1,
            "hypercode-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running hypercode -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        self.app_event_tx
            .send(AppEvent::HypercodeOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                sandbox_policy: None,
    ...
    pub(crate) fn open_approvals_popup(&mut self) {
        ...
        #[cfg(target_os = "windows")]
        let windows_degraded_sandbox_enabled = hypercode_core::get_platform_sandbox().is_some()
            && !hypercode_core::is_windows_elevated_sandbox_enabled();
        #[cfg(not(target_os = "windows"))]
        let windows_degraded_sandbox_enabled = false;

        let show_elevate_sandbox_hint = hypercode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    if hypercode_core::get_platform_sandbox().is_none() {
                        let preset_clone = preset.clone();
                        if hypercode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && hypercode_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.hypercode_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match hypercode_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.hypercode_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.sandbox_policy.get(),
            Some(self.config.hypercode_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(&mut self, preset: ApprovalPreset) {
        ...
        let info_line = Line::from(vec![
            "When Hypercode runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
        ...
        if !hypercode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    ...
                    line!["Learn more: https://developers.openai.com/hypercode/windows"],
                ])
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        lines.push(line![
            "Learn more: https://developers.openai.com/hypercode/windows"
        ]);
    ...
    pub(crate) fn maybe_prompt_windows_sandbox_enable(&mut self) {
        if self.config.forced_auto_mode_downgraded_on_windows
            && hypercode_core::get_platform_sandbox().is_none()
            && let Some(preset) = builtin_approval_presets()
                .into_iter()
                .find(|preset| preset.id == "auto")
        {
    ...
    pub(crate) fn set_sandbox_policy(&mut self, policy: SandboxPolicy) -> ConstraintResult<()> {
        ...
        let should_clear_downgrade = !matches!(&policy, SandboxPolicy::ReadOnly)
            || hypercode_core::get_platform_sandbox().is_some();
    ...
    /// Forward an `Op` directly to hypercode.
    pub(crate) fn submit_op(&mut self, op: Op) {
        ...
        if let Err(e) = self.hypercode_op_tx.send(op) {
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx: &AppEventSender| {
                tx.send(AppEvent::HypercodeOp(Op::Review {
                    review_request: ReviewRequest {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::HypercodeOp(Op::Review {
                        review_request: ReviewRequest {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = hypercode_core::git_info::recent_commits(cwd, 100).await;
...
async fn fetch_rate_limits(base_url: String, auth: HypercodeAuth) -> Option<RateLimitSnapshot> {
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<hypercode_core::git_info::CommitLogEntry>,
) {
