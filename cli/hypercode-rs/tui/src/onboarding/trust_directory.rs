...
use hypercode_core::config::set_project_trust_level;
use hypercode_core::git_info::resolve_root_git_project_for_trust;
...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(Line::from(vec![
            "> ".into(),
            "You are running Hypercode in ".bold(),
            self.cwd.to_string_lossy().to_string().into(),
        ]));
        column.push("");

        let guidance = if self.is_git_repo {
            "Since this folder is version controlled, you may wish to allow Hypercode to work in this folder without asking for approval."
        } else {
        ...
        if self.is_git_repo {
            options.push((
                "Yes, allow Hypercode to work in this folder without asking for approval",
            ...
            options.push((
                "Allow Hypercode to work in this folder without asking for approval",
                TrustDirectorySelection::Trust,
            ));
            options.push((
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trust_level(&self.hypercode_home, &target, TrustLevel::Trusted) {
    ...
    fn handle_dont_trust(&mut self) {
        ...
        let target =
            resolve_root_git_project_for_trust(&self.cwd).unwrap_or_else(|| self.cwd.clone());
        if let Err(e) = set_project_trust_level(&self.hypercode_home, &target, TrustLevel::Untrusted) {
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let hypercode_home = TempDir::new().expect("temp home");
