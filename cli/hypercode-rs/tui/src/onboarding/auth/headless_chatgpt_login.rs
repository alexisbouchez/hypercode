...
use hypercode_login::complete_device_code_login;
use hypercode_login::request_device_code;
                    ...
                    match run_login_server(opts) {
                        Ok(child) => {
                            ...
                            match r {
                                ...
                                _ => {
                                    *sign_in_state.write().unwrap() = SignInState::PickOpenAIMethod(OpenAIAuthMethod::ChatGPT);
                                    request_frame.schedule_frame();
                                }
                            }
                        }
...
mod tests {
    ...
    fn device_code_attempt_matches_only_for_matching_cancel() {
        ...
        assert_eq!(
            device_code_attempt_matches(&SignInState::PickOpenAIMethod(OpenAIAuthMethod::ChatGPT), &cancel),
            false
        );
    }
    ...
    fn begin_device_code_attempt_sets_state() {
        let sign_in_state = Arc::new(RwLock::new(SignInState::PickOpenAIMethod(OpenAIAuthMethod::ChatGPT)));
