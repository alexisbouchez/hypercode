...
use hypercode_core::is_dangerous_command::command_might_be_dangerous;
...
const CODEX_EXECVE_WRAPPER_EXE_NAME: &str = "hypercode-execve-wrapper";
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let hypercode_home = find_hypercode_home().context("failed to resolve hypercode_home for execpolicy")?;

    // TODO(mbolin): At a minimum, `cwd` should be configurable via
    // `hypercode/sandbox-state/update` or some other custom MCP call.
    let cwd = None;
    let cli_overrides = Vec::new();
    let overrides = hypercode_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = hypercode_core::config_loader::load_config_layers_state(
        &hypercode_home,
        cwd,
        &cli_overrides,
        overrides,
    ...
    hypercode_core::load_exec_policy(&config_layer_stack)
        .await
