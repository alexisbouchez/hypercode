...
use anyhow::Result;
use anyhow::ensure;
use hypercode_exec_server::ExecResult;
...
async fn accept_elicitation_for_prompt_rule() -> Result<()> {
    ...
    // $HYPERCODE_HOME with an execpolicy that prompts for `git init` commands.
...
"#,
    ...
    let transport = create_transport(hypercode_home.as_ref(), dotslash_cache).await?;
    ...
    let hypercode_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let hypercode_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("hypercode-linux-sandbox");
        let hypercode_cli = ensure_hypercode_cli()?;
        symlink(&hypercode_cli, &hypercode_linux_sandbox_exe)?;
        Some(hypercode_linux_sandbox_exe)
    } else {
        None
    };
    let response =
        notify_readable_sandbox(&project_root_path, hypercode_linux_sandbox_exe, &service).await?;
...
fn ensure_hypercode_cli() -> Result<PathBuf> {
    let hypercode_cli = hypercode_utils_cargo_bin::cargo_bin("hypercode")?;

    let metadata = hypercode_cli.metadata().with_context(|| {
        format!(
            "failed to read metadata for hypercode binary at {}",
            hypercode_cli.display()
        )
    })?;
    ensure!(
        metadata.is_file(),
        "expected hypercode binary at {} to be a file; run `cargo build -p hypercode-cli --bin hypercode` before this test",
        hypercode_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "hypercode binary at {} is not executable (mode {mode:o}); run `cargo build -p hypercode-cli --bin hypercode` before this test",
        hypercode_cli.display()
    );
