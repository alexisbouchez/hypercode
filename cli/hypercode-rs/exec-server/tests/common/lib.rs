use hypercode_core::MCP_SANDBOX_STATE_METHOD;
use hypercode_core::SandboxState;
...
where
    P: AsRef<Path>,
{
    let mcp_executable = hypercode_utils_cargo_bin::cargo_bin("hypercode-exec-mcp-server")?;
    let execve_wrapper = hypercode_utils_cargo_bin::cargo_bin("hypercode-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("HYPERCODE_HOME", hypercode_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, hypercode_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = hypercode_home.as_ref().join("rules");
