...
/// Applies the latest diff from a Hypercode agent task.
#[derive(Debug, Parser)]
...
pub async fn run_apply_command(
    ...
    init_chatgpt_token_from_auth(&config.hypercode_home, config.cli_auth_credentials_store_mode)
        .await?;
...
async fn apply_diff(diff: &str, cwd: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let res = hypercode_git::apply_git_patch(&req)?;
    if res.exit_code != 0 {
        anyhow::bail!(
