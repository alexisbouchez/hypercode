...
pub fn generate_json(out_dir: &Path) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("hypercode_app_server_protocol.schemas.json"),
        &bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_string(),
        Value::String("HypercodeAppServerProtocol".into()),
    );
...
mod tests {
    ...
    fn generated_ts_has_no_optional_nullable_fields() -> Result<()> {
        ...
        let output_dir = std::env::temp_dir().join(format!("hypercode_ts_types_{}", Uuid::now_v7()));
