...
use hypercode_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
use hypercode_protocol::config_types::ForcedLoginMethod;
use hypercode_protocol::config_types::ReasoningSummary;
use hypercode_protocol::config_types::SandboxMode as CoreSandboxMode;
use hypercode_protocol::config_types::Verbosity;
use hypercode_protocol::config_types::WebSearchMode;
use hypercode_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use hypercode_protocol::items::TurnItem as CoreTurnItem;
use hypercode_protocol::models::ResponseItem;
use hypercode_protocol::openai_models::ReasoningEffort;
use hypercode_protocol::parse_command::ParsedCommand as CoreParsedCommand;
use hypercode_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
use hypercode_protocol::plan_tool::StepStatus as CorePlanStepStatus;
use hypercode_protocol::protocol::AgentStatus as CoreAgentStatus;
use hypercode_protocol::protocol::AskForApproval as CoreAskForApproval;
use hypercode_protocol::protocol::HypercodeErrorInfo as CoreHypercodeErrorInfo;
use hypercode_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use hypercode_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use hypercode_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use hypercode_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use hypercode_protocol::protocol::SessionSource as CoreSessionSource;
use hypercode_protocol::protocol::SkillErrorInfo as CoreSkillErrorInfo;
use hypercode_protocol::protocol::SkillMetadata as CoreSkillMetadata;
use hypercode_protocol::protocol::SkillScope as CoreSkillScope;
use hypercode_protocol::protocol::TokenUsage as CoreTokenUsage;
use hypercode_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
...
/// This translation layer make sure that we expose hypercode error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `hypercodeErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum HypercodeErrorInfo {
...
impl From<CoreHypercodeErrorInfo> for HypercodeErrorInfo {
    fn from(value: CoreHypercodeErrorInfo) -> Self {
        match value {
            CoreHypercodeErrorInfo::ContextWindowExceeded => HypercodeErrorInfo::ContextWindowExceeded,
            CoreHypercodeErrorInfo::UsageLimitExceeded => HypercodeErrorInfo::UsageLimitExceeded,
            CoreHypercodeErrorInfo::HttpConnectionFailed { http_status_code } => {
                HypercodeErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreHypercodeErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                HypercodeErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreHypercodeErrorInfo::InternalServerError => HypercodeErrorInfo::InternalServerError,
            CoreHypercodeErrorInfo::Unauthorized => HypercodeErrorInfo::Unauthorized,
            CoreHypercodeErrorInfo::BadRequest => HypercodeErrorInfo::BadRequest,
            CoreHypercodeErrorInfo::ThreadRollbackFailed => HypercodeErrorInfo::ThreadRollbackFailed,
            CoreHypercodeErrorInfo::SandboxError => HypercodeErrorInfo::SandboxError,
            CoreHypercodeErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                HypercodeErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreHypercodeErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                HypercodeErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreHypercodeErrorInfo::Other => HypercodeErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from hypercode_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from hypercode_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// User config layer from $HYPERCODE_HOME/config.toml. This layer is special
    ...
    /// Path to a .hypercode/ folder within a project. There could be multiple of
    /// these between `cwd` and the project/repo root.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Project {
        dot_hypercode_folder: AbsolutePathBuf,
    },
...
impl SandboxPolicy {
    pub fn to_core(&self) -> hypercode_protocol::protocol::SandboxPolicy {
        match self {
            SandboxPolicy::DangerFullAccess => {
                hypercode_protocol::protocol::SandboxPolicy::DangerFullAccess
            }
            SandboxPolicy::ReadOnly => hypercode_protocol::protocol::SandboxPolicy::ReadOnly,
            SandboxPolicy::ExternalSandbox { network_access } => {
                hypercode_protocol::protocol::SandboxPolicy::ExternalSandbox {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                exclude_slash_tmp,
            } => hypercode_protocol::protocol::SandboxPolicy::WorkspaceWrite {
...
impl From<hypercode_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: hypercode_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            hypercode_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            hypercode_protocol::protocol::SandboxPolicy::ReadOnly => SandboxPolicy::ReadOnly,
            hypercode_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
pub struct ThreadStartParams {
    ...
    ///
    /// This is for internal use only (e.g. Hypercode Cloud).
...
pub struct Thread {
    ...
    /// Origin of the thread (CLI, VSCode, hypercode exec, hypercode app-server, etc.).
...
pub struct TurnError {
    pub message: String,
    pub hypercode_error_info: Option<HypercodeErrorInfo>,
    #[serde(default)]
...
mod tests {
    use super::*;
    use hypercode_protocol::items::AgentMessageContent;
    use hypercode_protocol::items::AgentMessageItem;
    use hypercode_protocol::items::ReasoningItem;
    use hypercode_protocol::items::TurnItem;
    use hypercode_protocol::items::UserMessageItem;
    use hypercode_protocol::items::WebSearchItem;
    use hypercode_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use hypercode_protocol::user_input::UserInput as CoreUserInput;
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.hypercode/skills/skill-creator/SKILL.md"),
                },
            ],
        });
    ...
    #[test]
    fn hypercode_error_info_serializes_http_status_code_in_camel_case() {
        let value = HypercodeErrorInfo::ResponseTooManyFailedAttempts {
