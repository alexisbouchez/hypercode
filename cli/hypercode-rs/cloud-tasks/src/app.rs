...
pub struct EnvironmentRow {
    ...
    pub is_pinned: bool,
    pub repo_hints: Option<String>, // e.g., "openai/hypercode"
}
...
use hypercode_cloud_tasks_client::CloudBackend;
...
pub struct AttemptView {
    ...
    pub status: hypercode_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: hypercode_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<hypercode_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<hypercode_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<hypercode_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    use super::*;
    use chrono::Utc;
    use hypercode_cloud_tasks_client::CloudTaskError;
    ...
    impl hypercode_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            &self,
            env: Option<&str>,
        ) -> hypercode_cloud_tasks_client::Result<Vec<TaskSummary>> {
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: hypercode_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: hypercode_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
        ...
        async fn get_task_summary(
            &self,
            id: TaskId,
        ) -> hypercode_cloud_tasks_client::Result<TaskSummary> {
        ...
        async fn get_task_diff(
            &self,
            _id: TaskId,
        ) -> hypercode_cloud_tasks_client::Result<Option<String>> {
            Err(hypercode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn get_task_messages(
            &self,
            _id: TaskId,
        ) -> hypercode_cloud_tasks_client::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn get_task_text(
            &self,
            _id: TaskId,
        ) -> hypercode_cloud_tasks_client::Result<hypercode_cloud_tasks_client::TaskText> {
            Ok(hypercode_cloud_tasks_client::TaskText {
                prompt: Some("Example prompt".to_string()),
                messages: Vec::new(),
                turn_id: Some("fake-turn".to_string()),
                sibling_turn_ids: Vec::new(),
                attempt_placement: Some(0),
                attempt_status: hypercode_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
        ...
        async fn list_sibling_attempts(
            ...
            _task: TaskId,
            _turn_id: String,
        ) -> hypercode_cloud_tasks_client::Result<Vec<hypercode_cloud_tasks_client::TurnAttempt>> {
            Ok(Vec::new())
        }
        ...
        ) -> hypercode_cloud_tasks_client::Result<hypercode_cloud_tasks_client::ApplyOutcome> {
            Err(hypercode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn create_task(
            ...
            _qa_mode: bool,
            _best_of_n: usize,
        ) -> hypercode_cloud_tasks_client::Result<hypercode_cloud_tasks_client::CreatedTask> {
            Err(hypercode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
