...
use hypercode_login::AuthManager;

pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = hypercode_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("hypercode_cloud_tasks_tui");
    let ua = hypercode_core::default_client::get_hypercode_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("hypercode-cli")),
    );
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/hypercode/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/hypercode") {
        return format!("{root}/hypercode/tasks/{task_id}");
    }
    if normalized.ends_with("/hypercode") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/hypercode/tasks/{task_id}")
}
