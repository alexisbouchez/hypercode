...
use anyhow::anyhow;
use chrono::Utc;
use hypercode_cloud_tasks_client::TaskStatus;
...
struct ApplyJob {
    task_id: hypercode_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn hypercode_cloud_tasks_client::CloudBackend>,
    base_url: String,
}
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(hypercode_cloud_tasks_client::MockClient),
            base_url,
        });
    }

    let ua = hypercode_core::default_client::get_hypercode_user_agent();
    let mut http = hypercode_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'hypercode login' to sign in with ChatGPT, then re-run 'hypercode cloud'."
            );
            std::process::exit(1);
        }
    };
...
impl GitInfoProvider for RealGitInfo {
    async fn default_branch_name(&self, path: &std::path::Path) -> Option<String> {
        hypercode_core::git_info::default_branch_name(path).await
    }

    async fn current_branch_name(&self, path: &std::path::Path) -> Option<String> {
        hypercode_core::git_info::current_branch_name(path).await
    }
}
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("hypercode_cloud_tasks_exec").await?;
    ...
    let created = hypercode_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `hypercode cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `hypercode cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn parse_task_id(raw: &str) -> anyhow::Result<hypercode_cloud_tasks_client::TaskId> {
    ...
    Ok(hypercode_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn hypercode_cloud_tasks_client::CloudBackend,
    task_id: &hypercode_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        hypercode_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        hypercode_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = hypercode_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &hypercode_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &hypercode_cloud_tasks_client::TaskSummary,
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("hypercode_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        hypercode_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("hypercode_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("hypercode_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = hypercode_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        hypercode_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: hypercode_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        hypercode_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        hypercode_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        hypercode_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = hypercode_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = hypercode_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `hypercode cloud` subcommand.
pub async fn run_main(cli: Cli, _hypercode_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext { backend, .. } = init_backend("hypercode_cloud_tasks_tui").await?;
    let backend = backend;
    ...
    append_error_log(format!(
        ...
        force_internal,
        hypercode_core::default_client::get_hypercode_user_agent()
    ));
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + hypercode_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match hypercode_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, hypercode_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                        ...
                                        // Ignore input while submitting
                                    } else if let hypercode_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = hypercode_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, false, best_of_n).await;
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match hypercode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match hypercode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = hypercode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match hypercode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
mod tests {
    ...
    use hypercode_cloud_tasks_client::DiffSummary;
    use hypercode_cloud_tasks_client::MockClient;
    use hypercode_cloud_tasks_client::TaskId;
    use hypercode_cloud_tasks_client::TaskStatus;
    use hypercode_cloud_tasks_client::TaskSummary;
    use hypercode_tui::ComposerAction;
    use hypercode_tui::ComposerInput;
    use crossterm::event::KeyCode;
    ...
    async fn collect_attempt_diffs_includes_sibling_attempts() {
        ...
        let task_id = parse_task_id("https://chatgpt.com/hypercode/tasks/T-1000").expect("id");
    ...
    fn parse_task_id_from_url_and_raw() {
        ...
        let url =
            parse_task_id("https://chatgpt.com/hypercode/tasks/task_i_123456?foo=bar").expect("url id");
