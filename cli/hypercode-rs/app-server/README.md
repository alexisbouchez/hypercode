# hypercode-app-server

`hypercode app-server` is the interface Hypercode uses to power rich interfaces such as the [Hypercode VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `hypercode app-server` supports bidirectional communication, streaming JSONL over stdio. The protocol is JSON-RPC 2.0, though the `"jsonrpc":"2.0"` header is omitted.

## Message Schema

Currently, you can dump a TypeScript version of the schema using `hypercode app-server generate-ts`, or a JSON Schema bundle via `hypercode app-server generate-json-schema`. Each output is specific to the version of Hypercode you used to run the command, so the generated artifacts are guaranteed to match that version.

```
hypercode app-server generate-ts --out DIR
hypercode app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Hypercode:

- **Thread**: A conversation between a user and the Hypercode agent. Each thread contains multiple turns.
...
- Initialize once: Immediately after launching the hypercode app-server process, send an `initialize` request with your client metadata, then emit an `initialized` notification. Any other request before this handshake gets rejected.
...
Applications building on top of `hypercode app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new Hypercode integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-Hypercode
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "hypercode_vscode",
      "title": "Hypercode VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `turn/start` — add user input to a thread and begin Hypercode generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications.
- `turn/interrupt` — request cancellation of an in-flight turn by `(thread_id, turn_id)`; success is an empty `{}` response and the turn finishes with `status: "interrupted"`.
- `review/start` — kick off Hypercode’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
Start a fresh thread when you need a new Hypercode conversation.
...
Turns attach user input (text or images) to a thread and trigger Hypercode generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.hypercode/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
The server requests cancellations for running subprocesses, then emits a `turn/completed` event with `status: "interrupted"`. Rely on the `turn/completed` to know when Hypercode-side cleanup is done.

### Example: Request a code review

Use `review/start` to run Hypercode’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Hypercode will run).
...
Hypercode streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Hypercode will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `turn/completed` — `{ turn }` where `turn.status` is `completed`, `interrupted`, or `failed`; failures carry `{ error: { message, hypercodeErrorInfo?, additionalDetails? } }`.
...
- `compacted` - `{threadId, turnId}` when hypercode compacts the conversation history. This can happen automatically.
...
`error` event is emitted whenever the server hits an error mid-turn (for example, upstream model errors or quota limits). Carries the same `{ error: { message, hypercodeErrorInfo?, additionalDetails? } }` payload as `turn.status: "failed"` and may precede that terminal notification.

`hypercodeErrorInfo` maps to the `HypercodeErrorInfo` enum. Common values:
...
When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `hypercodeErrorInfo` variant.

## Approvals

Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Hypercode whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
{ "id": 25, "result": {
    "skills": [
        { "name": "skill-creator", "description": "Create or update a Hypercode skill" }
    ]
} }
```
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Hypercode can run without OpenAI credentials.
