...
async fn test_hypercode_jsonrpc_conversation_flow() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    // Temporary Hypercode home with config pointing at the mock server.
    ...
    create_config_toml(&hypercode_home, &server.uri())?;
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![hypercode_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await?;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("hypercode/event/exec_command_begin"),
    )
    .await??;
