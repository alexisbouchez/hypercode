...
use hypercode_protocol::protocol::AskForApproval;
use hypercode_protocol::protocol::RawResponseItemEvent;
use hypercode_protocol::protocol::SandboxPolicy;
...
async fn test_send_message_success() -> Result<()> {
    // Spin up a mock responses server that immediately ends the Hypercode turn.
    // Two Hypercode turns hit the mock model (session start + send-user-message). Provide two SSE responses.
    ...
    // Create a temporary Hypercode home with config pointing at the mock server.
...
async fn send_message(
    ...
    let raw_attempt = tokio::time::timeout(
        ...
        mcp.read_stream_until_notification_message("hypercode/event/raw_response_item"),
    )
    .await;
    assert!(
        raw_attempt.is_err(),
...
async fn test_send_message_session_not_found() -> Result<()> {
    // Start MCP without creating a Hypercode session
    let hypercode_home = TempDir::new()?;
    let mut mcp = McpProcess::new(hypercode_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn read_raw_response_item(mcp: &mut McpProcess, conversation_id: ThreadId) -> ResponseItem {
    // TODO: Switch to rawResponseItem/completed once we migrate to app server v2 in hypercode web.
    loop {
        ...
        .await
        .expect("hypercode/event/raw_response_item notification timeout")
        .expect("hypercode/event/raw_response_item notification resp");

        let serde_json::Value::Object(params) = raw_notification
            .params
            .expect("hypercode/event/raw_response_item should have params")
        else {
            panic!("hypercode/event/raw_response_item should have params");
        };
