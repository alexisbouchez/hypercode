...
fn create_config_toml(hypercode_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() -> Result<()> {
    ...
    create_config_toml(hypercode_home.path())?;
    login_with_api_key(
        hypercode_home.path(),
        "sk-test-key",
...
fn create_config_toml_forced_login(hypercode_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = hypercode_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_rejected_when_forced_api() -> Result<()> {
    let hypercode_home = TempDir::new()?;
    create_config_toml_forced_login(hypercode_home.path(), "api")?;
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let hypercode_home = TempDir::new()?;
    create_config_toml_forced_workspace(hypercode_home.path(), "ws-forced")?;
