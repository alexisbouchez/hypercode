...
async fn init_mcp(hypercode_home: &Path) -> Result<McpProcess> {
...
async fn list_threads(
    ...
    let request_id = mcp
        .send_thread_list_request(hypercode_app_server_protocol::ThreadListParams {
            cursor,
            limit,
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(hypercode_home.path())?;

    let mut mcp = init_mcp(hypercode_home.path()).await?;
...
fn create_minimal_config(hypercode_home: &std::path::Path) -> std::io::Result<()> {
