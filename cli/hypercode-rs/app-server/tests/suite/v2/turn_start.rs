...
use hypercode_app_server_protocol::PatchChangeKind;
...
use hypercode_core::protocol_config_types::ReasoningSummary;
...
const TEST_ORIGINATOR: &str = "hypercode_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    create_config_toml(hypercode_home.path(), &server.uri(), "never")?;

    let mut mcp = McpProcess::new(hypercode_home.path()).await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: TEST_ORIGINATOR.to_string(),
            title: Some("Hypercode VS Code Extension".to_string()),
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Hypercode turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        hypercode_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Hypercode turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = hypercode_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let hypercode_home = tmp.path().to_path_buf();
    ...
    create_config_toml(hypercode_home.as_path(), &server.uri(), "untrusted")?;

    let mut mcp = McpProcess::new(hypercode_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": hypercode_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("hypercode/event/task_complete"),
    )
    .await??;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(hypercode_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(hypercode_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let hypercode_home = tmp.path().join("hypercode_home");
    std::fs::create_dir(&hypercode_home)?;
    ...
    let server = create_mock_responses_server_sequence(responses).await;
    create_config_toml(&hypercode_home, &server.uri(), "untrusted")?;

    let mut mcp = McpProcess::new(&hypercode_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(hypercode_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(hypercode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![hypercode_app_server_protocol::FileUpdateChange {
...
async fn command_execution_notifications_include_process_id() -> Result<()> {
    ...
    let hypercode_home = TempDir::new()?;
    create_config_toml(hypercode_home.path(), &server.uri(), "never")?;
    let config_toml = hypercode_home.path().join("config.toml");
...
fn create_config_toml(
    ...
    let config_toml = hypercode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
