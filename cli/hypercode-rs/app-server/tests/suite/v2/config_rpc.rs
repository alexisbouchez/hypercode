...
use hypercode_app_server_protocol::ConfigEdit;
...
use hypercode_app_server_protocol::MergeStrategy;
use hypercode_app_server_protocol::RequestId;
use hypercode_app_server_protocol::SandboxMode;
use hypercode_app_server_protocol::ToolsV2;
use hypercode_app_server_protocol::WriteStatus;
use hypercode_core::config_loader::SYSTEM_CONFIG_TOML_FILE_UNIX;
...
fn write_config(hypercode_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
        hypercode_home.path().join("config.toml"),
        contents,
    )?)
}
...
"#,
    ...
    let hypercode_home_path = hypercode_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(hypercode_home_path.join("config.toml"))?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let hypercode_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", hypercode_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(hypercode_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let hypercode_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(hypercode_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[hypercode_app_server_protocol::ConfigLayer],
