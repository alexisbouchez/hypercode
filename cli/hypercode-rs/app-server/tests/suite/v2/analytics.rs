...
use hypercode_core::config::types::OtelExporterKind;
use hypercode_core::config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut hypercode_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = hypercode_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("hypercode_app_server"),
        false,
    )
