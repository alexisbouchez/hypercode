...
use hypercode_app_server_protocol::LoginAccountResponse;
...
use hypercode_login::login_with_api_key;
...
fn create_config_toml(hypercode_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(hypercode_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(hypercode_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(hypercode_home.path(), &[("OPENAI_API_KEY", None)]).await?;
