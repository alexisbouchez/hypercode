...
use hypercode_app_server_protocol::RateLimitSnapshot;
use hypercode_app_server_protocol::RateLimitWindow;
...
async fn get_account_rate_limits_requires_auth() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "hypercode account authentication required to read rate limits"
    );
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(hypercode_home.path(), &server_url)?;
...
fn write_chatgpt_base_url(hypercode_home: &Path, base_url: &str) -> std::io::Result<()> {
