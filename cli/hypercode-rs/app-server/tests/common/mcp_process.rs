...
use hypercode_app_server_protocol::CancelLoginChatGptParams;
use hypercode_app_server_protocol::ClientInfo;
use hypercode_app_server_protocol::ClientNotification;
...
pub const DEFAULT_CLIENT_NAME: &str = "hypercode-app-server-tests";

impl McpProcess {
    pub async fn new(hypercode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(hypercode_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = hypercode_utils_cargo_bin::cargo_bin("hypercode-app-server")
            .context("should find binary for hypercode-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.env("HYPERCODE_HOME", hypercode_home);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("hypercode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
