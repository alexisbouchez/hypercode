...
- The legacy rule matcher lives in `hypercode-execpolicy-legacy`.
...
- From the Hypercode CLI, run `hypercode execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
hypercode execpolicy check --rules path/to/policy.rules git status
```
...
```bash
cargo run -p hypercode-execpolicy -- check --rules path/to/policy.rules git status
```
