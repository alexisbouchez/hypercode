...
use hypercode_backend_client as backend;
use hypercode_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/hypercode/tasks/{}", self.base_url, id.0),
            };
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let req = hypercode_git::ApplyGitRequest {
                cwd: std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()),
                diff: diff.clone(),
                revert: false,
                preflight,
            };
            let r = hypercode_git::apply_git_patch(&req)
    ...
    fn details_path(base_url: &str, id: &str) -> Option<String> {
        ...
        } else if base_url.contains("/api/hypercode") {
