...
use hypercode_utils_image::load_and_resize_to_fit;
...
use hypercode_utils_image::error::ImageProcessingError;
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Hypercode could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Hypercode cannot attach image at `{}`: unsupported image format `{}`.",
            path.display(),
            mime
        ),
    }
...
// (Moved event mapping logic into hypercode-core to avoid coupling protocol to UI-facing events.)
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Hypercode cannot attach image at `{}`: unsupported image format `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
