//! Defines the protocol for a Hypercode session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Hypercode:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::hypercode_thread::HypercodeThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request Hypercode to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Hypercode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down hypercode instance.
    Shutdown,
...
/// running the command proposed by Hypercode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.hypercode`, `.git`, notably `.git/hooks`) under
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        #[allow(clippy::expect_used)]
                        let top_level_hypercode = writable_root
                            .join(".hypercode")
                            .expect(".hypercode is a valid relative path");
                        if top_level_hypercode.as_path().is_dir() {
                            subpaths.push(top_level_hypercode);
                        }
                        WritableRoot {
                            root: writable_root,
...
/// Hypercode errors that we expose to clients.
