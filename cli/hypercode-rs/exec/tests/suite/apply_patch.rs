...
/// While we may add an `apply-patch` subcommand to the `hypercode` CLI multitool
/// at some point, we must ensure that the smaller `hypercode-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(hypercode_utils_cargo_bin::cargo_bin("hypercode-exec")?)
