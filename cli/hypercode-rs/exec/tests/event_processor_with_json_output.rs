...
use hypercode_exec::event_processor_with_jsonl_output::EventProcessorWithJsonOutput;
use hypercode_exec::exec_events::AgentMessageItem;
use hypercode_exec::exec_events::CommandExecutionItem;
use hypercode_exec::exec_events::CommandExecutionStatus;
use hypercode_exec::exec_events::ErrorItem;
use hypercode_exec::exec_events::ItemCompletedEvent;
use hypercode_exec::exec_events::ItemStartedEvent;
use hypercode_exec::exec_events::ItemUpdatedEvent;
use hypercode_exec::exec_events::McpToolCallItem;
use hypercode_exec::exec_events::McpToolCallItemError;
use hypercode_exec::exec_events::McpToolCallItemResult;
use hypercode_exec::exec_events::McpToolCallStatus;
use hypercode_exec::exec_events::PatchApplyStatus;
use hypercode_exec::exec_events::PatchChangeKind;
use hypercode_exec::exec_events::ReasoningItem;
use hypercode_exec::exec_events::ThreadErrorEvent;
use hypercode_exec::exec_events::ThreadEvent;
use hypercode_exec::exec_events::ThreadItem;
use hypercode_exec::exec_events::ThreadItemDetails;
use hypercode_exec::exec_events::ThreadStartedEvent;
use hypercode_exec::exec_events::TodoItem as ExecTodoItem;
use hypercode_exec::exec_events::TodoListItem as ExecTodoListItem;
use hypercode_exec::exec_events::TurnCompletedEvent;
use hypercode_exec::exec_events::TurnFailedEvent;
use hypercode_exec::exec_events::TurnStartedEvent;
use hypercode_exec::exec_events::Usage;
use hypercode_exec::exec_events::WebSearchItem;
...
use hypercode_protocol::protocol::ExecCommandOutputDeltaEvent;
use hypercode_protocol::protocol::ExecOutputStream;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_id =
        hypercode_protocol::ThreadId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8").unwrap();
    let rollout_path = PathBuf::from("/tmp/rollout.json");
    let ev = event(
        "e1",
        EventMsg::SessionConfigured(SessionConfiguredEvent {
            session_id,
            model: "hypercode-mini-latest".to_string(),
...
fn task_started_produces_turn_started_event() {
    ...
    let out = ep.collect_thread_events(&event(
        "t1",
        EventMsg::TurnStarted(hypercode_core::protocol::TurnStartedEvent {
...
fn plan_update_emits_todo_list_started_updated_and_completed() {
    ...
    let complete = event(
        "p3",
        EventMsg::TurnComplete(hypercode_core::protocol::TurnCompleteEvent {
...
fn error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::Error(hypercode_core::protocol::ErrorEvent {
...
fn stream_error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::StreamError(hypercode_core::protocol::StreamErrorEvent {
...
fn task_complete_produces_turn_completed_with_usage() {
    ...
    let usage = hypercode_core::protocol::TokenUsage {
        input_tokens: 1200,
    ...
    let info = hypercode_core::protocol::TokenUsageInfo {
    ...
    let token_count_event = event(
        "e1",
        EventMsg::TokenCount(hypercode_core::protocol::TokenCountEvent {
            info: Some(info),
            rate_limits: None,
        }),
    );
