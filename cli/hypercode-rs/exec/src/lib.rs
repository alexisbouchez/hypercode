...
use hypercode_core::LMSTUDIO_OSS_PROVIDER_ID;
use hypercode_core::NewThread;
use hypercode_core::OLLAMA_CHAT_PROVIDER_ID;
use hypercode_core::OLLAMA_OSS_PROVIDER_ID;
...
use hypercode_core::git_info::get_git_repo_root;
...
use hypercode_protocol::approvals::ElicitationAction;
...
use crate::event_processor::HypercodeStatus;
...
pub async fn run_main(cli: Cli, hypercode_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("hypercode_exec".to_string()) {
        tracing::warn!(?err, "Failed to set hypercode exec originator override {err:?}");
    }
    ...
    let config_toml = {
        let hypercode_home = match find_hypercode_home() {
            Ok(hypercode_home) => hypercode_home,
            Err(err) => {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        hypercode_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, false)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
    ...
    let (initial_operation, prompt_summary) = match (command, prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = hypercode_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Hypercode
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);

    info!("Hypercode initialized with event: {session_configured:?}");
            ...
            loop {
                tokio::select! {
                    _ = tokio::signal::ctrl_c() => {
                        ...
                        // Immediately notify Hypercode to abort any in‑flight task.
                        thread.submit(Op::Interrupt).await.ok();

                        // Exit the inner loop and return to the main input prompt. The hypercode
    ...
    while let Some(event) = rx.recv().await {
        ...
        let shutdown: HypercodeStatus = event_processor.process_event(event);
        match shutdown {
            HypercodeStatus::Running => continue,
            HypercodeStatus::InitiateShutdown => {
                thread.submit(Op::Shutdown).await?;
            }
            HypercodeStatus::Shutdown => {
                break;
            }
        }
    }
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match hypercode_core::RolloutRecorder::list_threads(
        ...
        let path = find_thread_path_by_id_str(&config.hypercode_home, id_str).await?;
        Ok(path)
    } else {
        Ok(None)
    }
