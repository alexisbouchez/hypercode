...
use hypercode_core::config::types::ShellEnvironmentPolicy;
use hypercode_core::error::HypercodeErr;
use hypercode_core::error::SandboxErr;
use hypercode_core::exec::ExecParams;
use hypercode_core::exec::process_exec_tool_call;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> hypercode_core::exec::ExecToolCallOutput {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_hypercode-linux-sandbox");
    let hypercode_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_write_blocked(cmd: &[&str], writable_roots: &[PathBuf], timeout_ms: u64) {
    ...
    match result {
        ...
        Err(HypercodeErr::Sandbox(SandboxErr::Denied { .. })) => {}
...
#[tokio::test]
async fn test_hypercode_dir_write_blocked() {
    let tmpdir = tempfile::tempdir().unwrap();
    let repo_root = tmpdir.path();
    std::fs::create_dir_all(repo_root.join(".hypercode")).unwrap();

    let hypercode_config = repo_root.join(".hypercode").join("config.toml");

    assert_write_blocked(
        &[
            "bash",
            "-lc",
            &format!("echo pwned > {}", hypercode_config.to_string_lossy()),
        ],
        &[repo_root.to_path_buf()],
        LONG_TIMEOUT_MS,
    )
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_hypercode-linux-sandbox");
    let hypercode_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    let result = process_exec_tool_call(
        params,
        &sandbox_policy,
        sandbox_cwd.as_path(),
    ...
    let output = match result {
        Ok(output) => output,
        Err(HypercodeErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
