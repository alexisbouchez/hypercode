...
use hypercode_core::error::Result;
use hypercode_core::protocol::SandboxPolicy;
use hypercode_core::protocol::WritableRoot;
    ...
    for writable_root in writable_roots {
        for ro_subpath in &writable_root.read_only_subpaths {
            ...
            if !ro_subpath.as_path().exists() {
                return Err(HypercodeErr::UnsupportedOperation(format!(
                    "Sandbox expected to protect {path}, but it does not exist. Ensure the repository contains this path or create it before running Hypercode.",
...
fn resolve_gitdir_from_file(dot_git: &AbsolutePathBuf) -> Result<AbsolutePathBuf> {
    let contents = std::fs::read_to_string(dot_git.as_path()).map_err(HypercodeErr::from)?;
    let trimmed = contents.trim();
    let (_, gitdir_raw) = trimmed.split_once(':').ok_or_else(|| {
        HypercodeErr::UnsupportedOperation(format!(
...
fn make_mounts_private() -> Result<()> {
    let root = CString::new("/").map_err(|_| {
        HypercodeErr::UnsupportedOperation("Sandbox mount path contains NUL byte: /".to_string())
    })?;
    let result = unsafe {
        libc::mount(
            std::ptr::null(),
...
mod tests {
    ...
    fn collect_read_only_mount_targets_errors_on_missing_path() {
        ...
        let message = match err {
            HypercodeErr::UnsupportedOperation(message) => message,
