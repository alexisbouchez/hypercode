WARNING: this code is mainly generated by Hypercode and should not be used in production

# hypercode-debug-client

A tiny interactive client for `hypercode app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `hypercode app-server` itself):

```
cargo run -p hypercode-debug-client -- \
  --hypercode-bin hypercode \
...
```
cargo run -p hypercode-debug-client -- --thread-id thr_123
```

### CLI flags

- `--hypercode-bin <path>`: path to the `hypercode` binary (default: `hypercode`).
- `-c, --config key=value`: pass through `--config` overrides to `hypercode`.
