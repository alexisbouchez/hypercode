use hypercode_client::Request;
use hypercode_client::RequestCompression;
use hypercode_client::RetryOn;
use hypercode_client::RetryPolicy;
use http::Method;
...
///
/// This is converted into a `RetryPolicy` used by `hypercode-client` to drive
...
impl Provider {
    ...
    /// Returns true if this provider supports the "developer" role in messages.
    /// Currently only OpenAI supports this role; other providers (like Mistral)
    /// need it mapped to "system".
    pub fn supports_developer_role(&self) -> bool {
        ...
        // OpenAI and Azure OpenAI support the developer role
        self.name.eq_ignore_ascii_case("openai")
            || self.name.eq_ignore_ascii_case("azure")
            || self.base_url.contains("openai.com")
            || self.base_url.contains("openai.azure.")
    }

    /// Returns true if this provider is Mistral AI.
    pub fn is_mistral(&self) -> bool {
        self.base_url.contains("mistral.ai") || self.name.eq_ignore_ascii_case("mistral")
    }
}
