...
use hypercode_protocol::models::FunctionCallOutputContentItem;
...
impl<'a> ChatRequestBuilder<'a> {
    ...
    pub fn build(self, provider: &Provider) -> Result<ChatRequest, ApiError> {
        ...
        // Whether to map developer role to system (for providers that don't support it)
        let supports_developer = provider.supports_developer_role();
        ...
        for (idx, item) in input.iter().enumerate() {
            match item {
                ResponseItem::Message { role, content, .. } => {
                    ...
                    let final_role = if role == "developer" && !supports_developer {
                        "system"
                    } else {
                        role.as_str()
                    };
                    let mut msg = json!({"role": final_role, "content": content_value});
                    if role == "assistant"
        ...
        // Format tools for the provider - Mistral doesn't accept top-level "name" field
        let tools: Vec<Value> = if provider.is_mistral() {
            self.tools
                .iter()
                .map(|tool| {
