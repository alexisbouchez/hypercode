...
Typed clients for Hypercode/OpenAI APIs built on top of the generic transport in `hypercode-client`.
...
- Serves as the wire-level layer consumed by `hypercode-core`; higher layers handle auth refresh and business logic.
...
- **Prompted endpoints (Chat + Responses)**
  - Input: a single `Prompt` plus endpoint-specific options.
    - `Prompt` (re-exported as `hypercode_api::Prompt`) carries:
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `hypercode_api::CompactionInput`):
    - `model: &str`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `hypercode-api` and `hypercode-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
