...
/// Regression test for https://github.com/openai/hypercode/issues/8803.
#[tokio::test]
async fn malformed_rules_should_not_panic() -> anyhow::Result<()> {
    // run_hypercode_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
...
"#,
    ...
    std::fs::write(hypercode_home.join("config.toml"), config_contents)?;

    let HypercodeCliOutput { exit_code, output } = run_hypercode_cli(hypercode_home, cwd).await?;
    assert_ne!(0, exit_code, "Hypercode CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize hypercode:"),
...
struct HypercodeCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_hypercode_cli(
    hypercode_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<HypercodeCliOutput> {
    let hypercode_cli = hypercode_utils_cargo_bin::cargo_bin("hypercode")?;
    let mut env = HashMap::new();
    env.insert(
        "HYPERCODE_HOME".to_string(),
        hypercode_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = hypercode_utils_pty::spawn_pty_process(
        hypercode_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            ...
            anyhow::bail!("timed out waiting for hypercode CLI to exit");
        }
    };
