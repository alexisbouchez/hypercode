...
This document describes a correctness bug in `hypercode-rs/tui2` and the chosen fix:
...
- `hypercode-rs/tui2/src/markdown_render.rs`
...
- `hypercode-rs/tui2/src/markdown_stream.rs`
...
- `hypercode-rs/tui2/src/streaming/controller.rs`
  - Emits `AgentMessageCell::new_logical(...)` containing logical lines.

- `hypercode-rs/tui2/src/history_cell.rs`
...
- `hypercode-rs/tui2/src/app_backtrack.rs`
...
the terminal (see `hypercode-rs/tui2/src/tui.rs`). This section is therefore best read as “behavior we
...
`hypercode-rs/tui2/docs/tui_viewport_and_history.md`.
...
- `AgentMessageCell` (`hypercode-rs/tui2/src/history_cell.rs`): **was affected**; now stores logical
  markdown lines and wraps at render time.
- `UserHistoryCell` (`hypercode-rs/tui2/src/history_cell.rs`): wraps at render time from stored `String`
  using `word_wrap_lines_with_joiners` (reflowable).
- `ReasoningSummaryCell` (`hypercode-rs/tui2/src/history_cell.rs`): renders from stored `String` on each
  call; it does call `append_markdown(..., Some(width))`, but that wrapping is recomputed per width
  (reflowable).
- `PrefixedWrappedHistoryCell` (`hypercode-rs/tui2/src/history_cell.rs`): wraps at render time and
  returns joiners (reflowable).
- `PlainHistoryCell` (`hypercode-rs/tui2/src/history_cell.rs`): stores `Vec<Line>` and returns it
...
- Printed transcript after exit (`hypercode-rs/tui2/src/app.rs`): `AppExitInfo.session_lines` is rendered
  once using the final width and then printed; it cannot reflow afterward.
- Optional scrollback insertion helper (`hypercode-rs/tui2/src/insert_history.rs`): once ANSI is written
  ...
  loop.
- Static overlays (`hypercode-rs/tui2/src/pager_overlay.rs`): reflow depends on whether callers provided
