//! Transcript/history cells for the Hypercode TUI.
//!
...
use base64::Engine;
use hypercode_common::format_env_display::format_env_display;
...
use hypercode_core::protocol::McpAuthStatus;
...
///
/// In `hypercode-tui`, `HistoryCell` only exposes `transcript_lines(...)` and the UI generally does not
/// need to reconstruct clipboard text across off-screen history or soft-wrap boundaries. In
/// `hypercode-tui2`, transcript selection and copy are app-driven (not terminal-driven) and may span
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openai/hypercode".cyan().underlined(),
        ...
        let content = text![
            ...
            "https://github.com/openai/hypercode/releases/latest"
                .cyan()
                .underlined(),
        ];
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: hypercode_core::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Hypercode".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/approvals".into(),
                " - choose what Hypercode can do without approval".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI Hypercode (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("OpenAI Hypercode").bold(),
            Span::from(" ").dim(),
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://github.com/openai/hypercode/blob/main/docs/config.md#mcp_servers\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
...
mod tests {
    ...
    use hypercode_core::config::types::McpServerConfig;
    use hypercode_core::config::types::McpServerTransportConfig;
    use hypercode_core::protocol::McpAuthStatus;
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved hypercode to".to_string(),
