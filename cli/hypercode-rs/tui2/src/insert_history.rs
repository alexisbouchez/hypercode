...
//! `hypercode-rs/tui2/src/tui.rs`). This module is still used for deterministic ANSI emission via
...
mod tests {
    ...
    fn write_spans_emits_truecolor_and_indexed_sgr() {
        ...
        // `hypercode-rs/clippy.toml`), but the test needs them so the output bytes are fully
        // deterministic.
