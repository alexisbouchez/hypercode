...
use hypercode_core::protocol::NetworkAccess;
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run hypercode login to use ChatGPT)".to_string()
            }
            StatusAccountDisplay::Mistral => {
                "Mistral API key configured".to_string()
            }
        });
        ...
        let note_first_line = Line::from(vec![
            ...
            "https://chatgpt.com/hypercode/settings/usage"
                .cyan()
                .underlined(),
