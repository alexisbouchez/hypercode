...
use hypercode_core::DEFAULT_LMSTUDIO_PORT;
use hypercode_core::DEFAULT_OLLAMA_PORT;
...
use hypercode_core::config::set_default_oss_provider;
...
pub async fn select_oss_provider(hypercode_home: &std::path::Path) -> io::Result<String> {
    ...
    if let Ok(ref provider) = result
        && let Err(e) = set_default_oss_provider(hypercode_home, provider)
    {
