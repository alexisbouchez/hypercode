...
const BASE_BUG_ISSUE_URL: &str =
    "https://github.com/openai/hypercode/issues/new?template=2-bug-report.yml";
...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: hypercode_feedback::HypercodeLogSnapshot,
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "hypercode-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Hypercode session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = hypercode_feedback::HypercodeFeedback::new().snapshot(None);
