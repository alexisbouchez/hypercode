...
impl ChatComposerHistory {
    ...
    fn populate_history_at_index(
        ...
        if global_idx >= self.history_entry_count {
            ...
            app_event_tx.send(AppEvent::HypercodeOp(op));
        }
        None
    }
}
...
mod tests {
    ...
    fn navigation_with_async_fetch() {
        ...
        // Verify that an AppEvent::HypercodeOp with the correct GetHistoryEntryRequest was sent.
        let event = rx.try_recv().expect("expected AppEvent to be sent");
        let AppEvent::HypercodeOp(history_request1) = event else {
        ...
        // Verify second HypercodeOp event for offset 1.
        let event2 = rx.try_recv().expect("expected second event");
        let AppEvent::HypercodeOp(history_request_2) = event2 else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
