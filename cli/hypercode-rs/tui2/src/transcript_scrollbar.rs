...
//!
//! The transcript in `hypercode-tui2` is rendered as a flattened list of wrapped visual lines. The
...
//! it matches `hypercode-tui2`’s existing "user prompt block" styling:
//!
...
//!
//! `hypercode-tui2` uses that helper rather than reimplementing scrollbar hit testing and drag math.
...
//!
//! `hypercode-tui2` uses the `ratatui` crate, while `tui-scrollbar` is built on `ratatui-core`.
...
//! `ratatui-core` crate. `hypercode-tui2` is currently pinned to an older Ratatui, so it still works
//! with `ratatui::buffer::Buffer` / `ratatui::layout::Rect`, while `tui-scrollbar` is already on
//! `ratatui-core`.
//!
//! That mismatch forces two bits of "glue" that should go away once `hypercode-tui2` upgrades to
//! Ratatui 0.30:
//!
...
///
/// `hypercode-tui2` reserves space for the transcript scrollbar even when it is not visible so the
...
///
/// Note that `hypercode-tui2` still reserves space for the scrollbar even when it is inactive; see
...
///   better match `hypercode-tui2`’s theme heuristics (see module docs).
/// - Because `tui-scrollbar` renders into a `ratatui-core` buffer while `hypercode-tui2` uses `ratatui`
