use clap::Parser;
use clap::ValueHint;
use hypercode_common::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `hypercode resume` subcommand.
    // These are not exposed as user flags on the base `hypercode` command.
    #[clap(skip)]
    ...
    /// top-level `hypercode resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `hypercode fork` subcommand.
    // These are not exposed as user flags on the base `hypercode` command.
    #[clap(skip)]
    ...
    /// top-level `hypercode fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<hypercode_common::SandboxModeCliArg>,
...
impl From<hypercode_tui::Cli> for Cli {
    fn from(cli: hypercode_tui::Cli) -> Self {
        Self {
            prompt: cli.prompt,
            images: cli.images,
