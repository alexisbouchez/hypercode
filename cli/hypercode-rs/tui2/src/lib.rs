...
// The standalone `hypercode-tui` binary prints a short help message before the
...
use hypercode_common::oss::ensure_oss_provider_ready;
use hypercode_common::oss::get_default_model_for_oss_provider;
use hypercode_common::oss::ollama_chat_deprecation_notice;
...
use hypercode_core::auth::enforce_login_restrictions;
...
use hypercode_core::config::resolve_oss_provider;
use hypercode_core::find_thread_path_by_id_str;
use hypercode_core::get_platform_sandbox;
use hypercode_core::protocol::AskForApproval;
use hypercode_core::terminal::Multiplexer;
...
pub async fn run_main(
    ...
    #[allow(clippy::print_stderr)]
    let hypercode_home = match find_hypercode_home() {
        Ok(hypercode_home) => hypercode_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding hypercode home: {err}");
            std::process::exit(1);
        }
    };
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&hypercode_home).await?;
    ...
    let log_dir = hypercode_core::config::log_dir(&config)?;
    ...
    // Hypercode CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("hypercode-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for hypercode crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("hypercode_core=info,hypercode_tui=info,hypercode_rmcp_client=info")
        })
    };
    ...
    let feedback = hypercode_feedback::HypercodeFeedback::new();
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        hypercode_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, true)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
            #[allow(clippy::print_stderr)]
    ...
    let terminal_info = hypercode_core::terminal::terminal_info();
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: hypercode_core::protocol::TokenUsage::default(),
    ...
    let auth_manager = AuthManager::shared(
        initial_config.hypercode_home.clone(),
        false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `hypercode {action}` without an ID to choose from existing sessions."
            )),
            session_lines: Vec::new(),
        })
    };
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            match find_thread_path_by_id_str(&config.hypercode_home, id_str).await? {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let hypercode_home = config.hypercode_home.clone();
        match HypercodeAuth::from_auth_storage(&hypercode_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use hypercode_core::config::ProjectConfig;
    use serial_test::serial;
    use tempfile::TempDir;

    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .hypercode_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
