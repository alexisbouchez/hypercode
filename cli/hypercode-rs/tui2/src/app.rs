...
use hypercode_ansi_escape::ansi_escape_line;
...
use hypercode_core::models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use hypercode_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
use hypercode_core::protocol::DeprecationNoticeEvent;
use hypercode_core::protocol::EventMsg;
use hypercode_core::protocol::FinalOutput;
...
use hypercode_core::protocol::SkillErrorInfo;
use hypercode_core::protocol::TokenUsage;
use hypercode_core::terminal::terminal_info;
use hypercode_protocol::ThreadId;
use hypercode_protocol::openai_models::ModelPreset;
use hypercode_protocol::openai_models::ModelUpgrade;
...
impl From<AppExitInfo> for hypercode_tui::AppExitInfo {
    fn from(info: AppExitInfo) -> Self {
        let exit_reason = match info.exit_reason {
            ExitReason::UserRequested => hypercode_tui::ExitReason::UserRequested,
            ExitReason::Fatal(message) => hypercode_tui::ExitReason::Fatal(message),
        };
        hypercode_tui::AppExitInfo {
...
fn session_summary(
    ...
    let resume_command =
        conversation_id.map(|conversation_id| format!("hypercode resume {conversation_id}"));
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_hypercode_max_migration_prompt
            .unwrap_or(false),
...
impl App {
    ...
    pub async fn run(
        ...
        #[cfg(target_os = "windows")]
        {
            let should_check = hypercode_core::get_platform_sandbox().is_some()
                && matches!(
                    app.config.sandbox_policy.get(),
                    hypercode_core::protocol::SandboxPolicy::WorkspaceWrite { .. }
                        | hypercode_core::protocol::SandboxPolicy::ReadOnly
                )
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.hypercode_home.clone();
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::HypercodeEvent(event) => {
                ...
                self.chat_widget.handle_hypercode_event(event);
            }
            ...
            AppEvent::HypercodeOp(op) => self.chat_widget.submit_op(op),
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let hypercode_home = self.config.hypercode_home.clone();
                    let tx = self.app_event_tx.clone();

                    // If the elevated setup already ran on this machine, don't prompt for
                    // elevation again - just flip the config to use the elevated path.
                    if hypercode_core::windows_sandbox::sandbox_setup_is_complete(hypercode_home.as_path())
                    {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = hypercode_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                            hypercode_home.as_path(),
                        );
                        ...
                        .await
                    {
                        Ok(()) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.app_event_tx.send(AppEvent::HypercodeOp(
            ...
            AppEvent::UpdateSandboxPolicy(policy) => {
                ...
                #[cfg(target_os = "windows")]
                if !matches!(&policy, hypercode_core::protocol::SandboxPolicy::ReadOnly)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.hypercode_home.clone();
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.hypercode_home)
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("hypercode-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> hypercode_core::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: hypercode_core::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = hypercode_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    fn all_model_presets() -> Vec<ModelPreset> {
        hypercode_core::models_manager::model_presets::all_model_presets().clone()
    }
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        let hypercode_home = tempdir().expect("temp hypercode home");
        let config = ConfigBuilder::default()
            .hypercode_home(hypercode_home.path().to_path_buf())
            .build()
            .await
            .expect("config");
    ...
    async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
        ...
        app.chat_widget.handle_hypercode_event(Event {
            id: String::new(),
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("hypercode resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
}
