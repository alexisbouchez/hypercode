...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = hypercode_home.clone();

    let opts = ServerOptions {
        hypercode_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: hypercode_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = hypercode_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_hypercode_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let hypercode_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_hypercode_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        hypercode_home: first_hypercode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: hypercode_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_hypercode_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        hypercode_home: second_hypercode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: hypercode_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
