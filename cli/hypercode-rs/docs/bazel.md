# Bazel in hypercode-rs

This repository uses Bazel to build the Rust workspace under `hypercode-rs`.
...
- `rules_rs` imports third-party crates from `hypercode-rs/Cargo.toml` and
  `hypercode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `hypercode_rust_crate`, which wraps `rust_library`,
...
- Each crate in `hypercode-rs/*/BUILD.bazel` typically uses `hypercode_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `hypercode_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `hypercode_rust_crate` to configure it.
