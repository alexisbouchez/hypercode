# Hypercode MCP Server Interface [experimental]

This document describes Hypercode’s experimental MCP server interface: a JSON‑RPC API that runs over the Model Context Protocol (MCP) transport to control a local Hypercode engine.

- Status: experimental and subject to change without notice
- Server binary: `hypercode mcp-server` (or `hypercode-mcp-server`)
- Transport: standard MCP over stdio (JSON‑RPC 2.0, line‑delimited)

## Overview

Hypercode exposes a small set of MCP‑compatible methods to create and manage conversations, send user input, receive live events, and handle approval prompts. The types are defined in `protocol/src/mcp_protocol.rs` and re‑used by the MCP server implementation in `mcp-server/`.

At a glance:

- Conversations
  - `newConversation` → start a Hypercode session
...
- Notifications (server → client)
  ...
  - `hypercode/event` stream with agent events

See code for full type definitions and exact shapes: `protocol/src/mcp_protocol.rs`.

## Starting the server

Run Hypercode as an MCP server and connect an MCP client:

```bash
hypercode mcp-server | your_mcp_client
```
...
Use the separate `hypercode mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current Hypercode build with `model/list`. The request accepts optional pagination inputs:
...
- `hypercode/event` with the serialized Hypercode event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
When Hypercode needs approval to apply changes or run commands, the server issues JSON‑RPC requests to the client:
...
```json
{ "jsonrpc": "2.0", "id": 2, "method": "sendUserMessage", "params": { "conversationId": "c7b0…", "items": [{ "type": "text", "text": "Hello Hypercode" }] } }
```

While processing, the server emits `hypercode/event` notifications containing agent output, approvals, and status updates.
