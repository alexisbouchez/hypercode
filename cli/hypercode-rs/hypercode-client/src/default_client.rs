...
#[derive(Clone, Debug)]
pub struct HypercodeHttpClient {
    inner: reqwest::Client,
}

impl HypercodeHttpClient {
    pub fn new(inner: reqwest::Client) -> Self {
        Self { inner }
    }

    pub fn get<U>(&self, url: U) -> HypercodeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn post<U>(&self, url: U) -> HypercodeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn request<U>(&self, method: Method, url: U) -> HypercodeRequestBuilder
    where
        U: IntoUrl,
    {
        let url_str = url.as_str().to_string();
        HypercodeRequestBuilder::new(self.inner.request(method.clone(), url), method, url_str)
    }
}

#[must_use = "requests are not sent unless `send` is awaited"]
#[derive(Debug)]
pub struct HypercodeRequestBuilder {
    builder: reqwest::RequestBuilder,
    method: Method,
    url: String,
}

impl HypercodeRequestBuilder {
