...
# Run a suite of smoke tests against the Windows sandbox via the Hypercode CLI
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the Hypercode CLI to invoke `codex sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Hypercode.
    """
    ...
    raise FileNotFoundError(
        "Hypercode CLI not found. Build it first, e.g.\n"
