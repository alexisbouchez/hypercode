...
pub const OFFLINE_USERNAME: &str = "HypercodeSandboxOffline";
pub const ONLINE_USERNAME: &str = "HypercodeSandboxOnline";
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;

pub fn sandbox_dir(hypercode_home: &Path) -> PathBuf {
    hypercode_home.join(".sandbox")
}

pub fn setup_marker_path(hypercode_home: &Path) -> PathBuf {
    sandbox_dir(hypercode_home).join("setup_marker.json")
}

pub fn sandbox_users_path(hypercode_home: &Path) -> PathBuf {
    sandbox_dir(hypercode_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| hypercode_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(hypercode_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("hypercode-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("hypercode-windows-sandbox-setup.exe")
}
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(hypercode_home);
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, hypercode_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, hypercode_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to HYPERCODE_HOME or anything under HYPERCODE_HOME/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let hypercode_home_key = crate::audit::normalize_path_key(hypercode_home);
    let sbx_dir_key = crate::audit::normalize_path_key(&sandbox_dir(hypercode_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = crate::audit::normalize_path_key(root);
        key != hypercode_home_key && key != sbx_dir_key && !key.starts_with(&sbx_dir_prefix)
    });
    roots
}
