...
use anyhow::Context;
use anyhow::Result;
use hypercode_windows_sandbox::allow_null_device;
use hypercode_windows_sandbox::convert_string_sid_to_sid;
use hypercode_windows_sandbox::create_process_as_user;
use hypercode_windows_sandbox::create_readonly_token_with_cap_from;
use hypercode_windows_sandbox::create_workspace_write_token_with_cap_from;
use hypercode_windows_sandbox::get_current_token_for_restriction;
use hypercode_windows_sandbox::hide_current_user_profile_dir;
use hypercode_windows_sandbox::log_note;
use hypercode_windows_sandbox::parse_policy;
use hypercode_windows_sandbox::to_wide;
use hypercode_windows_sandbox::SandboxPolicy;
...
struct RunnerRequest {
    ...
    // Writable location for logs (sandbox user's .hypercode).
    hypercode_home: PathBuf,
    // Real user's HYPERCODE_HOME for shared data (caps, config).
    real_hypercode_home: PathBuf,
    cap_sid: String,
    command: Vec<String>,
    cwd: PathBuf,
...
pub fn main() -> Result<()> {
    ...
    let log_dir = Some(req.hypercode_home.as_path());
    hide_current_user_profile_dir(req.hypercode_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_hypercode_home={}",
            req.cwd.display(),
            req.command,
            req.real_hypercode_home.display()
        ),
        Some(&req.hypercode_home),
    );
