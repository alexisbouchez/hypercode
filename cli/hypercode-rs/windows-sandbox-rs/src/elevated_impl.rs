mod windows_impl {
    ...
    /// Creates the sandbox user's Hypercode home directory if it does not already exist.
    fn ensure_hypercode_home_exists(p: &Path) -> Result<()> {
    ...
    /// Locates `hypercode-command-runner.exe` next to the current binary.
    fn find_runner_exe() -> PathBuf {
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let candidate = dir.join("hypercode-command-runner.exe");
                if candidate.exists() {
                    return candidate;
                }
            }
        }
        PathBuf::from("hypercode-command-runner.exe")
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_entropy();
        format!(r"\\.\pipe\hypercode-runner-{:x}-{}", rng.gen::<u128>(), suffix)
    }
    ...
    struct RunnerPayload {
        ...
        // Writable log dir for sandbox user (.hypercode in sandbox profile).
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = hypercode_home.join(".sandbox");
        ensure_hypercode_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, hypercode_home)?;
        ...
        let caps = load_or_create_cap_sids(hypercode_home)?;
        ...
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "hypercode-command-runner.exe".to_string());
        ...
        let payload = RunnerPayload {
            ...
            hypercode_home: sandbox_base.clone(),
            real_hypercode_home: hypercode_home.to_path_buf(),
