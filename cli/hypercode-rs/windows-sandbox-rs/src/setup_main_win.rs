...
use hypercode_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use hypercode_windows_sandbox::ensure_allow_write_aces;
use hypercode_windows_sandbox::hide_newly_created_users;
use hypercode_windows_sandbox::load_or_create_cap_sids;
use hypercode_windows_sandbox::log_note;
use hypercode_windows_sandbox::path_mask_allows;
use hypercode_windows_sandbox::sandbox_dir;
use hypercode_windows_sandbox::string_from_sid_bytes;
use hypercode_windows_sandbox::to_wide;
use hypercode_windows_sandbox::LOG_FILE_NAME;
use hypercode_windows_sandbox::SETUP_VERSION;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(hypercode_home) = std::env::var("HYPERCODE_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&hypercode_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.hypercode_home);
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.hypercode_home)?;
    let cap_psid = unsafe {
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.hypercode_home),
            &payload.real_user,
            &sandbox_group_sid,
            log,
        )?;
