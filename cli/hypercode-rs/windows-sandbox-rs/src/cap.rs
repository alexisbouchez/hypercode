...
pub fn cap_sid_file(hypercode_home: &Path) -> PathBuf {
    hypercode_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(hypercode_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(hypercode_home);
    if path.exists() {
