...
use hypercode_app_server_protocol::InitializeParams;
use hypercode_app_server_protocol::InitializeResponse;
use hypercode_app_server_protocol::InputItem;
use hypercode_app_server_protocol::JSONRPCMessage;
...
use hypercode_app_server_protocol::SandboxPolicy;
...
use hypercode_app_server_protocol::ServerRequest;
...
/// Minimal launcher that initializes the Hypercode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Hypercode", version, about = "Bootstrap Hypercode app-server", long_about = None)]
struct Cli {
    /// Path to the `hypercode` CLI binary.
    #[arg(long, env = "CODEX_BIN", default_value = "hypercode")]
    hypercode_bin: String,

    /// Forwarded to the `hypercode` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Send a user message through the Hypercode app-server.
    SendMessage {
        /// User message to send to Hypercode.
        #[arg()]
        user_message: String,
    },
    ...
    TestLogin,
    /// Fetch the current account rate limits from the Hypercode app-server.
    GetAccountRateLimits,
    /// List the available models from the Hypercode app-server.
...
fn main() -> Result<()> {
    ...
    match command {
        CliCommand::SendMessage { user_message } => {
            send_message(&hypercode_bin, &config_overrides, user_message)
        }
        CliCommand::SendMessageV2 { user_message } => {
            send_message_v2(&hypercode_bin, &config_overrides, user_message)
        }
        CliCommand::TriggerCmdApproval { user_message } => {
            trigger_cmd_approval(&hypercode_bin, &config_overrides, user_message)
        }
        CliCommand::TriggerPatchApproval { user_message } => {
            trigger_patch_approval(&hypercode_bin, &config_overrides, user_message)
        }
        CliCommand::NoTriggerCmdApproval => no_trigger_cmd_approval(&hypercode_bin, &config_overrides),
        ...
        CliCommand::TestLogin => test_login(&hypercode_bin, &config_overrides),
        CliCommand::GetAccountRateLimits => get_account_rate_limits(&hypercode_bin, &config_overrides),
        CliCommand::ModelList => model_list(&hypercode_bin, &config_overrides),
    }
}

fn send_message(hypercode_bin: &str, config_overrides: &[String], user_message: String) -> Result<()> {
    let mut client = HypercodeClient::spawn(hypercode_bin, config_overrides)?;
...
fn send_message_v2(
    ...
    send_message_v2_with_policies(hypercode_bin, config_overrides, user_message, None, None)
}
...
fn no_trigger_cmd_approval(hypercode_bin: &str, config_overrides: &[String]) -> Result<()> {
    let prompt = "Run `touch should_not_trigger_approval.txt`";
    send_message_v2_with_policies(hypercode_bin, config_overrides, prompt.to_string(), None, None)
}
...
fn test_login(hypercode_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = HypercodeClient::spawn(hypercode_bin, config_overrides)?;
...
fn get_account_rate_limits(hypercode_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = HypercodeClient::spawn(hypercode_bin, config_overrides)?;
...
fn model_list(hypercode_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = HypercodeClient::spawn(hypercode_bin, config_overrides)?;
...
impl HypercodeClient {
    fn spawn(hypercode_bin: &str, config_overrides: &[String]) -> Result<Self> {
        let mut cmd = Command::new(hypercode_bin);
        for override_kv in config_overrides {
            cmd.arg("--config").arg(override_kv);
        }
        let mut hypercode_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{hypercode_bin}` app-server"))?;

        let stdin = hypercode_app_server
            .stdin
            .take()
            .context("hypercode app-server stdin unavailable")?;
        let stdout = hypercode_app_server
            .stdout
            .take()
            .context("hypercode app-server stdout unavailable")?;

        Ok(Self {
            child: hypercode_app_server,
            stdin: Some(stdin),
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "hypercode-toy-app-server".to_string(),
                    title: Some("Hypercode Toy App Server".to_string()),
    ...
    fn remove_thread_listener(&mut self, subscription_id: Uuid) -> Result<()> {
        ...
        let request = ClientRequest::RemoveConversationListener {
            request_id: request_id.clone(),
            params: hypercode_app_server_protocol::RemoveConversationListenerParams { subscription_id },
        };

        self.send_request::<hypercode_app_server_protocol::RemoveConversationSubscriptionResponse>(
            request,
            request_id,
    ...
    fn stream_conversation(&mut self, conversation_id: &ThreadId) -> Result<()> {
        loop {
            ...
            if !notification.method.starts_with("hypercode/event/") {
                continue;
            }
    ...
    fn write_request(&mut self, request: &ClientRequest) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush request to hypercode app-server")?;
        } else {
            bail!("hypercode app-server stdin closed");
        }
    ...
    fn read_jsonrpc_message(&mut self) -> Result<JSONRPCMessage> {
        loop {
            ...
            let bytes = self
                ...
                .context("failed to read from hypercode app-server")?;

            if bytes == 0 {
                bail!("hypercode app-server closed stdout");
            }
    ...
    fn write_jsonrpc_message(&mut self, message: JSONRPCMessage) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush response to hypercode app-server")?;
            return Ok(());
        }
...
impl Drop for HypercodeClient {
    fn drop(&mut self) {
        let _ = self.stdin.take();

        if let Ok(Some(status)) = self.child.try_wait() {
            println!("[hypercode app-server exited: {status}]");
            return;
        }
