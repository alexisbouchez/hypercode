...
use hypercode_core::config::load_global_mcp_servers;
use hypercode_core::config::types::McpServerConfig;
use hypercode_core::config::types::McpServerTransportConfig;
use hypercode_core::mcp::auth::compute_auth_statuses;
use hypercode_core::protocol::McpAuthStatus;
use hypercode_rmcp_client::delete_oauth_tokens;
use hypercode_rmcp_client::perform_oauth_login;
use hypercode_rmcp_client::supports_oauth_login;

/// Subcommands:
/// - `serve`  — run the MCP server on stdio
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.hypercode/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "hypercode mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let hypercode_home = find_hypercode_home().context("failed to resolve HYPERCODE_HOME")?;
    let mut servers = load_global_mcp_servers(&hypercode_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", hypercode_home.display()))?;
    ...
    ConfigEditsBuilder::new(&hypercode_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", hypercode_home.display()))?;
    ...
    } = transport
    {
        match supports_oauth_login(&url).await {
            ...
            Ok(false) => {}
            Err(_) => println!(
                "MCP server may or may not require login. Run `hypercode mcp login {name}` to login."
            ),
        }
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `hypercode mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: hypercode mcp remove {}", get_args.name);
