...
use hypercode_core::auth::logout;
use hypercode_core::config::Config;
use hypercode_login::ServerOptions;
use hypercode_login::run_device_code_login;
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | hypercode login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `hypercode login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match HypercodeAuth::from_auth_storage(&config.hypercode_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => match auth.mode {
            AuthMode::ApiKey => match auth.get_token() {
                Ok(api_key) => {
                    eprintln!("Logged in using an OpenAI API key - {}", safe_format_key(&api_key));
                    std::process::exit(0);
                }
                Err(e) => {
            ...
            AuthMode::Mistral => match auth.get_token() {
                Ok(api_key) => {
                    eprintln!("Logged in using a Mistral API key - {}", safe_format_key(&api_key));
                    std::process::exit(0);
                }
                Err(e) => {
                    eprintln!("Unexpected error retrieving Mistral API key: {e}");
                    std::process::exit(1);
                }
            },
            AuthMode::ChatGPT => {
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout(&config.hypercode_home, config.cli_auth_credentials_store_mode) {
        Ok(true) => {
