...
pub use hypercode_core::env::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\hypercode.zip").as_deref(),
            Some("/mnt/c/Temp/hypercode.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/hypercode.tgz").as_deref(),
            Some("/mnt/d/Work/hypercode.tgz")
        );
        assert!(win_path_to_wsl("/home/user/hypercode").is_none());
    }
