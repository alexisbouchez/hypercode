# Performance testing (`hypercode-tui2`)

This doc captures a repeatable workflow for investigating `hypercode-tui2` performance issues
...
- Human: runs `hypercode-tui2` in an interactive terminal (e.g. Ghostty), triggers “idle” and
...
```sh
cd hypercode-rs
cargo build -p hypercode-tui2
```

Run `hypercode-tui2` in a terminal and get a PID (macOS):

```sh
pgrep -n hypercode-tui2
```

Track CPU quickly while reproducing:

```sh
top -pid "$(pgrep -n hypercode-tui2)"
```
...
```sh
sample "$(pgrep -n hypercode-tui2)" 1 -file /tmp/tui2.idle.sample.txt
sample "$(pgrep -n hypercode-tui2)" 1 -file /tmp/tui2.streaming.sample.txt
```
...
- `hypercode-rs/tui2/src/transcript_view_cache.rs`: wrapped transcript memoization + per-line
  rasterization cache (cached `Cell` rows).
- `hypercode-rs/tui2/src/transcript_render.rs`: incremental helper used by the wrapped-line cache
  (`append_wrapped_transcript_cell`).
- `hypercode-rs/tui2/src/app.rs`: wiring in `App::render_transcript_cells` (uses cached rows instead of
