...
This document explains the design decision behind Hypercode's alternate screen handling, particularly in terminal multiplexers like Zellij. This addresses a fundamental conflict between fullscreen TUI behavior and terminal scrollback history preservation.

## The Problem

### Fullscreen TUI Benefits

Hypercode's TUI uses the terminal's **alternate screen buffer** to provide a clean fullscreen experience. This approach:
...
When using Hypercode's TUI in Zellij, users cannot scroll back through the conversation history because:
...
Hypercode implements a **pragmatic workaround** with three modes, controlled by `tui.alternate_screen` in `config.toml`:
...
This detection happens in the helper function `determine_alt_screen_mode()` in `hypercode-rs/tui/src/lib.rs`.

### Configuration Schema

The `AltScreenMode` enum is defined in `hypercode-rs/protocol/src/config_types.rs` and serializes to lowercase TOML:
...
- **Original Issue:** [GitHub #2558](https://github.com/openai/hypercode/issues/2558) - "No scrollback in Zellij"
- **Implementation PR:** [GitHub #8555](https://github.com/openai/hypercode/pull/8555)
...
Hypercode's transcript pager (opened with Ctrl+T) provides an alternative way to review conversation history, even in fullscreen mode. However, this is not as seamless as natural scrollback.
...
If you encounter issues with terminal state after running Hypercode, you can restore your terminal with:
