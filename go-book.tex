...
\definecolor{roseDark}{HTML}{9F1239}
\definecolor{roseMedium}{HTML}{F43F5E}
\definecolor{roseLight}{HTML}{FDA4AF}
\definecolor{rosePale}{HTML}{FFF1F2}
\definecolor{bgCode}{HTML}{F7F7F7}
\definecolor{bgTask}{HTML}{FFF8E1}
\definecolor{bgSolution}{HTML}{E8F5E9}
\definecolor{frameCode}{HTML}{E5E5E5}
\definecolor{accentDark}{HTML}{1A1A1A}
\definecolor{chapterBg}{HTML}{E11D48}
...
\titleformat{\section}
  {\normalfont\LARGE\bfseries\color{roseDark}}
  {\thesection}{1em}{}

\titleformat{\subsection}
  {\normalfont\Large\bfseries\color{roseDark!80!black}}
  {\thesubsection}{1em}{}

\titleformat{\subsubsection}
  {\normalfont\large\bfseries\color{roseDark!60!black}}
  {\thesubsubsection}{1em}{}
...
% Decorative line
{\color{rose}\rule{\textwidth}{2pt}}

\vspace{1.5cm}

% Title
\includegraphics[width=0.75\textwidth]{public/nabla-hypercode.png}

\vspace{0.5cm}

{\fontsize{22}{28}\selectfont\color{roseDark} Learn Go from Scratch}

\vspace{1.5cm}

% Decorative line
{\color{rose}\rule{0.6\textwidth}{1pt}}

\vspace{2cm}

% Subtitle description
{\large\color{gray}
An interactive guide to the Go programming language.\\[6pt]
From ``Hello, World!'' to generics ---\\[6pt]
twelve lessons covering the fundamentals of Go.
}

\vspace{3cm}

% Author
{\Large\bfseries Alexis Bouchez}

\vspace{0.5cm}

{\color{gray!70}\large hypercode.alexisbouchez.com}
...
This book contains twelve lessons organized into seven chapters. Each lesson explains a concept, demonstrates it with code examples, and ends with an exercise for you to practice. Solutions are provided at the end of each lesson. The lessons are designed to be worked through in order, as each builds on the concepts introduced before it.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Data Structures} --- Slices (dynamic arrays) and maps (hash tables).
...
\begin{taskbox}
Write a function \code{safeDivide} that takes two \code{float64} parameters and returns two values: a \code{float64} result and a \code{string} error message.

\begin{itemize}[leftmargin=1.5em]
  \item If the second argument is 0, return \code{0} and \code{"division by zero"}
  \item Otherwise, return the result of the division and an empty string \code{""}
\end{itemize}
...
Implement \code{Area()} on both types. The area of a circle is $3.14159 \times r^2$.
...
Congratulations on completing all twelve lessons! You now have a solid foundation in Go's core language features. Here are some directions to explore next:

\subsection*{Concurrency}
Go's goroutines and channels are one of its most powerful features. Learn how to run functions concurrently with \code{go} and communicate between them with channels.
